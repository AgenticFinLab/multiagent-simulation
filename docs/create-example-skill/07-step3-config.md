...
**Reference**: Copy from `configs/AssetBubble/Rule/` and adapt.
...
market:
  name: "Market"
  class: "examples.{SimulationName}.{Variant}.players:Market"
  num_instances: 1
  config:
    identity: "market"
    role: coordinator
    extras:
      # Market parameters — see simulation-bases.md §3.1 and §6
...
{agent_type_1}:
  ...
  config:
    ...
    role: player
    extras:
      # Investor parameters — see simulation-bases.md §4.{N} and §6
...
# Star topology: Market center ↔ all investors bidirectional
...
- `bidirectional: true` enables both broadcast (market→investors) and orders (investors→market)
...
market:
  type: proxy
  checkpoint_dir: "EXPERIMENT/{SimulationName}/{Variant}/checkpoints/market"
  record_path: "EXPERIMENT/{SimulationName}/{Variant}/records/market"
  monitoring:
    record_path: "EXPERIMENT/{SimulationName}/{Variant}/records/market/monitor"
...
- Investor type behavior descriptions
...
- [ ] LLM/RuleLLM/Rag variants have appropriate `object_store_memory` (≥512MB)
- [ ] Rag variant has `rag:` block in every agent's `extras`
