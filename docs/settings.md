...
are passed to `ray.init()` in `masim/utils/ray_utils.py:ensure_ray()`.
...
| Field                 | Type        | Description                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|-----------------------|-------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `num_cpus`            | int         | Number of logical CPUs reserved for this Ray cluster. **Must be set explicitly** — do not use null. Derivation: `N_investors_in_largest_level + 1` (all investors run in parallel at topology level 1; +1 for OS/driver process). Source: `_resolve_num_cpus()` in `masim/utils/ray_utils.py`.                                                                                                                                               |
...
player_id:
  ...
  # REQUIRED: number of identical instances to spawn. Set 1 for a single instance.
  # Omitting num_instances is a config error — load_config() raises KeyError immediately.
  num_instances: 1
  config:
...
| `class`                                | string | Python import path to the player class in format `"module.path:ClassName"`. The framework calls `load_class(path)` (`masim/utils/config.py:load_class`) to dynamically import and instantiate the class.                                                                                                                                                                                           |
| `num_instances`                        | int    | **REQUIRED — no default.** Number of identical copies to spawn. `1` = single instance, key unchanged. `N > 1` = expander creates `key_1` … `key_N`, each with a suffixed `identity` and display name. Topology YAML uses the BASE key; the expander resolves instances before the simulator starts. Missing this field raises `KeyError` in `expand_player_instances()` (`masim/utils/config.py`). |
...
| `config.steps_per_turn`                | int    | Number of internal decision steps the player performs per `operate()` call. `1` = single perceive→decide→act(→on_fill) cycle per round. Reserved for multi-step agent architectures. The terminal `on_fill(action, quantity, bid_price)` hook fires only after the framework act has finalised the order; scenario code MUST NOT re-implement act to move this step (see `docs/framework-contract.md`).                                                                                                                                                                                                                         |
...
| Field         | Type   | Description                                                                                                                                                                                                                                                                                                                                   |
|---------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `type`        | string | Graph pattern: `"star"` (hub-and-spoke), `"mesh"` (fully connected), `"custom"` (arbitrary adjacency list). All current scenarios use `"star"` with market as hub. `validate_config()` (`masim/utils/config.py`) accepts these three values only.                                                                                             |
