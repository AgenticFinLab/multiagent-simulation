...
from examples.SorosPound.RuleLLM.prompts import format_user_prompt, get_prompt
from examples.SorosPound.Rule.players import Market

logger = logging.getLogger("SorosPound.RuleLLM")


class MacroHedgeFund(GeneralPlayer):
    """LLM-driven MacroHedgeFund. Builds massive short positions against currencies with unsustainable pegs"""
...
class PegDefender(GeneralPlayer):
    """LLM-driven PegDefender. Attempts to maintain currency peg through interest rate hikes and intervention"""
    def __init__(self, config=None):
        super().__init__(config)
        self.llm_client = None
        self.agent_type = ""
...
class ConvergenceTrader(GeneralPlayer):
    """LLM-driven ConvergenceTrader. Takes positions expecting the peg to hold, loses when it breaks"""
    def __init__(self, config=None):
        super().__init__(config)
        self.llm_client = None
        self.agent_type = ""
...
class OpportunisticTrader(GeneralPlayer):
    """LLM-driven OpportunisticTrader. Joins speculative attacks once they begin, amplifying selling pressure"""
    def __init__(self, config=None):
        super().__init__(config)
        self.llm_client = None
        self.agent_type = ""
