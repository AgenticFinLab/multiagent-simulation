"""SorosPound RuleLLM Simulation

Hybrid Rule+LLM variant of SorosPound.

Design:
    - Market: Rule-based coordinator (identical to SorosPound.Rule.Market).
    - Investors: LLM-powered with system prompts that encode BOTH persona
      AND explicit quantitative decision rules from the Rule variant.
      Agents use LangChainAPIInference with the masim standard pattern.
...
from .prompts import (
    RULELLM_MACRO_HEDGE_FUND_SYS,
    RULELLM_PEG_DEFENDER_SYS,
    RULELLM_CONVERGENCE_TRADER_SYS,
    RULELLM_OPPORTUNISTIC_TRADER_SYS,
    RULELLM_NOISE_TRADER_SYS,
)
from ..Rule.players import Market  # noqa: F401 — re-exported

logger = logging.getLogger("SorosPound.RuleLLM")


class RuleLLMInvestor(GeneralPlayer):
    """Base class for hybrid Rule+LLM SorosPound investors."""
...
class RuleLLMMacroHedgeFund(RuleLLMInvestor):
    """Rule+LLM macro hedge fund building short positions against unsustainable pegs."""

    _system_prompt = RULELLM_MACRO_HEDGE_FUND_SYS


class RuleLLMPegDefender(RuleLLMInvestor):
    """Rule+LLM central bank defending currency peg through reserves."""

    _system_prompt = RULELLM_PEG_DEFENDER_SYS


class RuleLLMConvergenceTrader(RuleLLMInvestor):
    """Rule+LLM convergence trader betting on peg stability."""

    _system_prompt = RULELLM_CONVERGENCE_TRADER_SYS


class RuleLLMOpportunisticTrader(RuleLLMInvestor):
    """Rule+LLM opportunistic trader joining speculative attacks."""

    _system_prompt = RULELLM_OPPORTUNISTIC_TRADER_SYS
