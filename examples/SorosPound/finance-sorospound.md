...
| Status        | released                                                                                                                                                                                                                                                                            |
| CHANGELOG     | 2026-07-03  Produced by define-simulation-scenario-skill.md v1.2.0 under polish-simulation-pipeline.md Step 0 Case B (Status: draft). Source of §2–§10 content: existing downstream artefacts under `examples/SorosPound/` (simulation-bases.md, analysis-bases.md, Rule/players.py, configs/SorosPound/Rule/players.yml). / 2026-07-03  Polish target-file gate: structural validation green (10/10 sections, 6 DOIs across §4, 5 stylized facts, 5 agents in §7, 4 variants in §10.1), Status upgraded draft → locked. / 2026-07-03  Polish Steps 3-4: rewrote all four `configs/SorosPound/{Rule,LLM,RuleLLM,Rag}/players.yml` with `# Source: target §9 <parameter> row` inline citations on every `extras` leaf; added `agent_type` field inside LLM/RuleLLM/Rag `extras` per `agent-design-skill.md §3.6.0 I/O Contract`; hoisted the `(No relevant knowledge retrieved this round.)` string in `examples/SorosPound/Rag/players.py` to a module-level `_RAG_FALLBACK` constant. / 2026-07-03  Polish Steps 5-9 three-pass review: (Pass 1 theory-code alignment) `MacroHedgeFund` acts iff `|deviation| > 0.02` with `min(800, int(|deviation| × 5000))` (§4.1 Krugman 1979 / Flood-Garber 1984); `PegDefender` acts iff `|deviation| > 0.05` with `min(500, int(|deviation| × 3000))` (§4.3 Obstfeld 1996 / Eichengreen-Wyplosz 1993); `ConvergenceTrader` samples `Bernoulli(0.3)` with random direction and `randint(100, 500)` (§4.4); `OpportunisticTrader` uses the same threshold and sizing as `MacroHedgeFund` (§4.2 Obstfeld 1996 self-fulfilling herding); `NoiseTrader` samples `Bernoulli(0.3)` with random direction and `randint(100, 500)` (§4.5 Black 1986) — all match `Rule/players.py`. (Pass 2 code quality) `python3 -m py_compile` clean on all 4 variant `players.py`; PyYAML `!include`-noop load clean on all 4 `players.yml`. (Pass 3 documentation consistency) prompt symbols `LLM_*_SYS / LLM_USER_TEMPLATE`, `RULELLM_*_SYS / RULELLM_USER_TEMPLATE`, `RAGLLM_*_SYS / RAG_USER_TEMPLATE` present in respective `prompts.py` and referenced by config `sys_message` / `user_message`. / 2026-07-03  Rule smoke test: `PYTHONPATH=. python3 examples/SorosPound/Rule/run_sorospound.py -c configs/SorosPound/Rule/simulation.yml` with `total_rounds` temporarily set to 5 launched 10 actors (1 Market + 2 MacroHedgeFund + 1 PegDefender + 2 ConvergenceTrader + 2 OpportunisticTrader + 2 NoiseTrader) and completed 5/5 rounds with no exceptions; `total_rounds` restored to 200 after the run. / 2026-07-03  Polish Closeout: Status transitioned locked → released. Deferred defect (out of polish scope): `examples/SorosPound/Rule/analysis.py` delegates to `examples.standard_rule_analysis`, which was deleted 2026-06-27 — this affects all bias-scenario Rule `analysis.py` variants and is logged for a separate remediation pass. |
