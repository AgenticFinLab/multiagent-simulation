#!/usr/bin/env python
"""MentalAccounting Rag analysis with standard outputs and RAG stats."""
...
def analyze_rag_knowledge_effect(
    ...
    """Calculate RAG retrieval coverage from recorded order payloads."""
...
def main(config_path: str | None = None) -> Dict[str, Any]:
    """Run MentalAccounting Rag analysis and write `rag_stats.json`."""
    if config_path is None:
        parser = argparse.ArgumentParser(description="MentalAccounting Rag analysis")
    ...
    summary = analyze_mentalaccounting(data, config, output_dir)
