#!/usr/bin/env python
"""MentalAccounting Simulation Analysis

Analyze the MentalAccounting simulation results.

Usage:
    python examples/MentalAccounting/Rule/analysis.py \
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("MentalAccounting Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "mentalaccounting_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze MentalAccounting simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/MentalAccounting/Rule/simulation.yml")
