...
| arb_threshold = 0.01, base_size = 80 → sim-bases §6              | `arb_threshold = float(extras["arb_threshold"])`; `position_size = int(extras["base_size"])` |
...
| Sell when deviation < -trigger_threshold with amplification → sim-bases §4 | `amplified = int(base_size * (1 + feedback_strength * abs(deviation) * 10))` |
| trigger_threshold = 0.01, feedback_strength = 1.2 → sim-bases §6           | Both loaded from `extras`                                                    |
| Buy when deviation > +trigger_threshold (trend-following) → sim-bases §4   | `buy_qty = min(base_size, int(cash / price))`                                |
...
| Buy when deviation < -value_discount → sim-bases §4                  | `if deviation < -value_discount: buy_qty = min(base_size, int(cash / price))` |
| value_discount = 0.15, base_size = 40 → sim-bases §6               | Both loaded from `extras`                                                      |
...
|----------------------|--------------------------------------|-------------------------------|-------|
| `λ` (price_impact)   | `price_impact`                       | `extras["price_impact"]`      | 0.05  |
| `γ` (mean_reversion) | `mean_reversion`                     | `extras["mean_reversion"]`    | 0.01  |
| `F` (fundamental)    | `fundamental`                        | `extras["fundamental_value"]` | 250.0 |
| `D(t)` (net demand)  | `net_demand = buy_vol − sell_vol`    | computed from incoming orders | —     |
| `ε(t)` (noise)       | `noise = random.gauss(0, noise_std)` | `extras["noise_std"]`         | 0.02  |
...
**Feedback amplification in code**: ProgramTrader's `amplified = int(base_size × (1 + feedback_strength × |deviation| × 10))` is the core feedback formula. As deviation deepens, sell orders grow exponentially, creating the self-reinforcing crash.

**Simultaneous triggers**: Multiple agents trigger simultaneously when thresholds are breached — PortfolioInsurer at 2%, ProgramTrader at 1%, IndexArbitrageur at 1%. This sequential activation creates wave structure.
...
**NoiseTrader as background signal**: With 5% trade probability and 50–200 share range, NoiseTrader provides realistic background activity that slightly perturbs cascade timing.
...
║    ├── P(t+1) = P(t) + 0.05×D + 0.01×(250−P) + N(0, 0.02²)          ║
║    └── deviation = (P(t+1) − 250) / 250                              ║
║                                                                       ║
...
║  IndexArbitrageur: |deviation| > 0.01? → SELL/BUY up to 80 shares       ║
║  ProgramTrader: deviation < -0.01?                                    ║
║    → SELL: amplified = base_size × (1 + 1.2 × |deviation| × 10)    ║
║  ValueInvestor: deviation < -0.15? → BUY up to 40                       ║
║  NoiseTrader: 5% chance → random 50–200 buy/sell                   ║
║         │                                                             ║
...
|-----------------------|------------------------------|-------|----------------------------------------------------------------------------|
| `price_impact`        | `extras.price_impact`        | 0.05  | Moderate λ — large lot sizes (hundreds of shares) create significant moves |
| `mean_reversion`      | `extras.mean_reversion`      | 0.01  | Moderate γ — allows crash persistence but not permanent                    |
| `fundamental_value`   | `extras.fundamental_value`   | 250.0 | Stable benchmark; no fundamental deterioration (crash is mechanical)       |
...
| `feedback_strength`   | `extras.feedback_strength`   | 1.2   | Amplification factor from Brady Commission analysis; see sim-bases §6      |
...
|------------------|--------|--------------------------------------------------------------------------------|------------------------------------------------------------|
| Pre-Crash        | 1–10   | All agents hold; IndexArbitrageur may make small trades on noise deviations    | Price near 250; small fluctuations from noise              |
