...
def main() -> None:
    """Run BlackMonday1987 Rag analysis: metrics + RAG knowledge-effect report.

    Implements analysis-bases.md §2 core metrics plus Rag-specific
    knowledge-retrieval analysis. Output written to
    EXPERIMENT/BlackMonday1987/Rag/records/analysis/.
    """
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        default="configs/BlackMonday1987/Rag/simulation.yml",
    )
    ...
    fig.suptitle(
        "BlackMonday1987 Rag Simulation Analysis", fontsize=14, fontweight="bold"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(analysis_path, "blackmonday1987_rag_analysis.png"), dpi=150
    )
    plt.close()
