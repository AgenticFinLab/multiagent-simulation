"""MarketCrashRag — RAG-augmented Rule+LLM Market Crash Simulation
...
This extends the three-variant comparison:
    ...
    MarketCrashRuleLLM — LLM with rules in prompt (no external knowledge)
    MarketCrashRag     — LLM with rules in prompt + RAG knowledge retrieval
...
1. **Via Streamlit Web UI (Recommended):**
   ...
   ```
   Then select "MarketCrashRag" from the scenario dropdown.

2. **Command Line:**

   ```bash
   python examples/MarketCrash/Rag/run_flash_crash_ragllm.py \
...
# =============================================================================
# Market — Rule-Based Coordinator (identical to MarketCrashRuleLLM.Market)
# =============================================================================
...
class RagLLMInvestor(GeneralPlayer):
    """
    Base class for RAG-augmented Rule+LLM market crash investors.
