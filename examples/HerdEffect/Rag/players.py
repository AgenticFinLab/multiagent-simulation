"""HerdEffectRag — RAG-augmented Rule+LLM Herd Effect Simulation

Design:
    - Market coordinator: identical rule-based price dynamics as HerdEffect.
...
This extends the three-variant comparison:
    ...
    HerdEffectRuleLLM — LLM with rules in prompt (no external knowledge)
    HerdEffectRag     — LLM with rules in prompt + RAG knowledge retrieval
...
1. **Via Streamlit Web UI (Recommended):**
   ...
   ```
   Then select "HerdEffectRag" from the scenario dropdown.

2. **Command Line:**

   ```bash
   python examples/HerdEffect/Rag/run_flash_crash_ragllm.py \
...
# =============================================================================
# Market — Rule-Based Coordinator (identical to HerdEffectRuleLLM.Market)
# =============================================================================
...
class Market(GeneralPlayer):
    ...
    Price model (rule-based, unchanged from HerdEffect):
        Price impact increases when liquidity is low (herding amplification mechanism).
...
class RagLLMInvestor(GeneralPlayer):
    """
    Base class for RAG-augmented Rule+LLM herd effect investors.
