...
Framing effect causes traders to make different decisions based on how equivalent information is presented
...
Key Dynamics:
- GainFrameFollower: Overweights gains-framed information, becomes risk-averse when returns are presented as gains
- LossFrameReactor: Overweights loss-framed information, becomes risk-seeking when presented with potential losses
- FrameInvariantTrader: Evaluates information by substance regardless of framing, computes equivalent outcomes
- ArbitrageFramer: Exploits framing-induced mispricing by recognizing when same data drives different prices
- NoiseTrader: Random uninformed trader providing baseline liquidity

Parameters from config (see configs/FramingEffect/Rule/players.yml):
"""
...
class GainFrameFollower(GeneralPlayer):
    ...
    def _make_decision(self, price: float, fundamental: float, deviation: float) -> dict:
        ...
        gain_weight = extras["gain_weight"]
        loss_weight = extras["loss_weight"]
