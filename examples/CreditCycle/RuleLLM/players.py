...
from examples.CreditCycle.RuleLLM.prompts import format_user_prompt, get_prompt
from examples.CreditCycle.Rule.players import Market

logger = logging.getLogger("CreditCycle.RuleLLM")


class ProCyclicalLender(GeneralPlayer):
    """LLM-driven ProCyclicalLender. Expands lending during booms, tightens during downturns, amplifying credit cycles"""
...
class MinskyBorrower(GeneralPlayer):
    """LLM-driven MinskyBorrower. Increases debt levels during stability, creating fragility that leads to crisis"""
    def __init__(self, config=None):
        super().__init__(config)
        self.llm_client = None
        self.agent_type = ""
...
class CounterCyclicalLender(GeneralPlayer):
    """LLM-driven CounterCyclicalLender. Lends counter-cyclically, providing liquidity during crises when others withdraw"""
    def __init__(self, config=None):
        super().__init__(config)
        self.llm_client = None
        self.agent_type = ""
...
class ValueInvestor(GeneralPlayer):
    """LLM-driven ValueInvestor. Invests based on fundamental value, providing stability during credit expansions"""
    def __init__(self, config=None):
        super().__init__(config)
        self.llm_client = None
        self.agent_type = ""
