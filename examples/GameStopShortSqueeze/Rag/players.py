"""GameStopShortSqueeze Rag — RAG-augmented LLM simulation of short squeeze dynamics."""
...
class RagLLMInvestor(GeneralPlayer):
    """Base RAG-augmented LLM investor for GameStopShortSqueeze."""
    ...
    def _build_prompt(self) -> str:
        ...
        if rag_store and rag_store.is_built():
            ...
            query = KnowledgeQuery(
                text=(
                    f"short squeeze retail coordination strategy "
...
class RagLLMRetailCoordinated(RagLLMInvestor):
    """RagLLM-driven retail coordinated trader: buys aggressively via social media coordination. Theory: simulation-bases.md §4.1."""

    _system_prompt_path = (
        "examples.GameStopShortSqueeze.Rag.prompts:RAGLLM_RETAIL_COORDINATED_SYS"
    )


class RagLLMShortSellerHF(RagLLMInvestor):
    """RagLLM-driven short seller hedge fund: maintains short positions under squeeze pressure. Theory: simulation-bases.md §4.2."""

    _system_prompt_path = (
        "examples.GameStopShortSqueeze.Rag.prompts:RAGLLM_SHORT_SELLER_HF_SYS"
    )


class RagLLMMarketMakerGamma(RagLLMInvestor):
    """RagLLM-driven market maker with gamma hedging: delta-hedges options exposure. Theory: simulation-bases.md §4.3."""

    _system_prompt_path = (
        "examples.GameStopShortSqueeze.Rag.prompts:RAGLLM_MARKET_MAKER_GAMMA_SYS"
    )


class RagLLMInstitutionalValue(RagLLMInvestor):
    """RagLLM-driven institutional value investor: trades on fundamentals, fades the squeeze. Theory: simulation-bases.md §4.4."""

    _system_prompt_path = (
        "examples.GameStopShortSqueeze.Rag.prompts:RAGLLM_INSTITUTIONAL_VALUE_SYS"
    )


class RagLLMMomentumRetail(RagLLMInvestor):
    """RagLLM-driven momentum retail trader: chases price momentum without coordination. Theory: simulation-bases.md §4.5."""

    _system_prompt_path = (
        "examples.GameStopShortSqueeze.Rag.prompts:RAGLLM_MOMENTUM_RETAIL_SYS"
    )
...
__all__ = [
    "Market",
    "RagLLMInvestor",
    "RagLLMRetailCoordinated",
    "RagLLMShortSellerHF",
    "RagLLMMarketMakerGamma",
    "RagLLMInstitutionalValue",
    "RagLLMMomentumRetail",
]
