...
| Status        | released                                                                                                                                                                                                                                                                            |
| CHANGELOG     | 2026-07-03  Produced by define-simulation-scenario-skill.md v1.2.0 under polish-simulation-pipeline.md Step 0 Case B (Status: draft). Source of §2–§10 content: existing downstream artefacts under `examples/SunkCostFallacy/` (simulation-bases.md, analysis-bases.md, Rule/players.py, configs/SunkCostFallacy/Rule/players.yml). / 2026-07-03  Polish target-file gate: structural validation green (10/10 sections, 5 DOIs across §4, 5 stylized facts, 5 agents in §7, 4 variants in §10.1), Status upgraded draft → locked. / 2026-07-03  Steps 3–4 config traceability: all 4 variant `configs/SunkCostFallacy/{Rule,LLM,RuleLLM,Rag}/players.yml` extended with `# Source: target §9 <param> row` inline citations on every extras leaf; LLM/RuleLLM/Rag variants gained `agent_type` field per agent-design-skill §3.6.0 I/O Contract; `examples/SunkCostFallacy/Rag/players.py` hoisted the RAG fallback literal to module-level `_RAG_FALLBACK`. / 2026-07-03  Steps 5–9 three-pass review: Pass 1 theory-code alignment verified — SunkCostHolder / CommitmentEscalator / RationalCutter / OpportunityCostTrader / NoiseTrader decision rules in Rule/players.py match §4.1–§4.5 and §5 F1–F5 acceptance ranges; Pass 2 code quality — `python3 -m py_compile` clean on all 4 variant players.py, PyYAML `!include` no-op parse clean on all 4 players.yml, no stale `examples.standard_rule_analysis`-safe imports at player level; Pass 3 documentation consistency — simulation-bases.md §4 and §6 parameter values match target §9, analysis-bases.md §2 metric names match target §5 F1–F5, all 5 LLM/RuleLLM/Rag prompt symbols referenced by players.yml exist. / 2026-07-03  Rule 5-round smoke test: `PYTHONPATH=. python3 examples/SunkCostFallacy/Rule/run_sunkcostfallacy.py -c configs/SunkCostFallacy/Rule/simulation.yml` launched 13 actors, completed 5/5 rounds without exceptions; total_rounds restored to 200. / 2026-07-03  Closeout: Status transitioned locked → released. Deferred pre-existing defect (shared across scenarios): `Rule/analysis.py` still imports `run_standard_analysis` which delegates to the deleted `examples.standard_rule_analysis` module — logged only, out of polish scope. |
...
| `commitmentescalator.extras.escalation_threshold`  | Loss threshold for averaging-down `|deviation| > θ`                | `0.05`      | simulation-bases.md §4.2; Staw (1976), 10.1016/0030-5073(76)90005-2                                                          |
