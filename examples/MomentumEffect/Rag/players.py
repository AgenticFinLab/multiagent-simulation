"""MomentumEffectRag — RAG-augmented Rule+LLM Momentum Effect Simulation
...
This extends the three-variant comparison:
    ...
    MomentumEffectRuleLLM — LLM with rules in prompt (no external knowledge)
    MomentumEffectRag     — LLM with rules in prompt + RAG knowledge retrieval
...
1. **Via Streamlit Web UI (Recommended):**
   ...
   ```
   Then select "MomentumEffectRag" from the scenario dropdown.

2. **Command Line:**

   ```bash
   python examples/MomentumEffect/Rag/run_flash_crash_ragllm.py \
