#!/usr/bin/env python
"""AnchoringEffect Simulation Analysis

Analyze the AnchoringEffect simulation results.
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("AnchoringEffect Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "anchoringeffect_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze AnchoringEffect simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/AnchoringEffect/Rule/simulation.yml")
