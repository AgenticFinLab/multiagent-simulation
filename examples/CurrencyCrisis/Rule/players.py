...
Self-fulfilling speculative currency attacks where market expectations of devaluation trigger the crisis itself
...
Key Dynamics:
- SpeculativeAttacker: Builds short positions in vulnerable currency, profiting from forced devaluation
- SelfFulfillingTrader: Sells currency based on expectation that others will sell, making the crisis inevitable
- CentralBankDefender: Defends currency peg using foreign reserves and interest rate adjustments
- FundamentalHedger: Hedges based on fundamental analysis rather than speculative attacks
- NoiseTrader: Random uninformed trader providing baseline liquidity

Parameters from config (see configs/CurrencyCrisis/Rule/players.yml):
"""
...
logger = logging.getLogger("CurrencyCrisis")


class Market(GeneralPlayer):
    """
    Market agent for CurrencyCrisis simulation.
...
class SpeculativeAttacker(GeneralPlayer):
    ...
    def _make_decision(self, price: float, fundamental: float, deviation: float) -> dict:
        ...
        attack_threshold = extras["attack_threshold"]
...
class SelfFulfillingTrader(GeneralPlayer):
    """
    Sells currency based on expectation that others will sell, making the crisis inevitable
    ...
    def _make_decision(self, price: float, fundamental: float, deviation: float) -> dict:
        ...
        cash = self.state.custom_state["cash"]
        position = self.state.custom_state["position"]
        contagion_sensitivity = extras["contagion_sensitivity"]
        exit_threshold = extras["exit_threshold"]
...
class CentralBankDefender(GeneralPlayer):
    ...
    def _make_decision(self, price: float, fundamental: float, deviation: float) -> dict:
        ...
        cash = self.state.custom_state["cash"]
        position = self.state.custom_state["position"]
        reserve_level = extras["reserve_level"]
        defense_threshold = extras["defense_threshold"]
...
class FundamentalHedger(GeneralPlayer):
    """
    Hedges based on fundamental analysis rather than speculative attacks
...
__all__ = ["Market, SpeculativeAttacker, SelfFulfillingTrader, CentralBankDefender, FundamentalHedger, NoiseTrader"]
