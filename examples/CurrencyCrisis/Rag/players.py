"""CurrencyCrisis Rag — RAG-augmented LLM simulation of currency crisis dynamics."""
...
class RagLLMInvestor(GeneralPlayer):
    """Base RAG-augmented LLM investor for CurrencyCrisis."""
    ...
    async def _initialize_agent(self) -> None:
        ...
        self.state.custom_state["history_buffer"] = HistoryBuffer(
            folder=f"CurrencyCrisis/Rag/{self.__class__.__name__}", entry_limit=200
        )
    ...
    def _build_prompt(self, market_data: Dict[str, Any]) -> str:
        ...
        if rag_store and rag_store.is_built():
            ...
            query = KnowledgeQuery(
                text=(
                    f"currency crisis speculative attack: price={price:.4f}, "
                    f"fundamental={fundamental:.4f}, deviation={deviation:+.2%}"
                ),
                top_k=top_k,
                round_num=round_num,
        ...
        template = load_prompt("examples.CurrencyCrisis.Rag.prompts:RAG_USER_TEMPLATE")
