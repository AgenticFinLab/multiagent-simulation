"""ReversalEffectRag — RAG-augmented Rule+LLM Reversal Effect Simulation

Design:
    - Market coordinator: identical rule-based price dynamics as ReversalEffect.
...
This extends the three-variant comparison:
    ...
    ReversalEffectRuleLLM — LLM with rules in prompt (no external knowledge)
    ReversalEffectRag     — LLM with rules in prompt + RAG knowledge retrieval
...
1. **Via Streamlit Web UI (Recommended):**
   ...
   ```
   Then select "ReversalEffectRag" from the scenario dropdown.

2. **Command Line:**

   ```bash
   python examples/ReversalEffect/Rag/run_flash_crash_ragllm.py \
...
# =============================================================================
# Market — Rule-Based Coordinator (identical to ReversalEffectRuleLLM.Market)
# =============================================================================
...
class Market(GeneralPlayer):
    ...
    Price model (rule-based, unchanged from ReversalEffect):
        Price impact increases when liquidity is low (reversal amplification mechanism).
