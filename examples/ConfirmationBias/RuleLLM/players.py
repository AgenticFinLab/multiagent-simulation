"""ConfirmationBias RuleLLM Simulation — LLM agents with explicit numerical trading rules."""
...
class RuleLLMInvestor(GeneralPlayer):
    """Base RuleLLM investor for ConfirmationBias."""
    ...
    async def _initialize_agent(self) -> None:
        ...
        self.state.custom_state["history_buffer"] = HistoryBuffer(
            folder=f"ConfirmationBias/RuleLLM/{self.__class__.__name__}",
            entry_limit=200,
        )
        load_dotenv()
        llm_cfg = extras["llm"]
    ...
    async def decide(self) -> Dict:
        ...
        user_template = load_prompt(
            "examples.ConfirmationBias.RuleLLM.prompts:RULELLM_USER_TEMPLATE"
        )
