...
- 2026-07-21  Polish Round 2 re-audit — all gates green. Preflight: scenario tree intact, `finance-gfc-2008.md` §1 Status = `released`. Step 0 target-file re-audit: 10-section canonical structure PASS, 5 §4 theory anchors with resolvable DOIs PASS, §5 F1–F4 acceptance metrics resolve to `analysis.py` surface, §7 five agent rows aligned to §4 theories, §9 11-row parameter grid unchanged, §10.1 four-variant `Yes` marks unchanged. Step 1 research: 5 DOIs resolve (Keys et al. 2010 `10.1162/qjec.2010.125.1.307`; Bolton et al. 2012 `10.1111/j.1540-6261.2011.01708.x`; Brunnermeier & Pedersen 2009 `10.1093/rfs/hhn098`; Griffin & Xu 2009 `10.1093/rfs/hhp003`; Adrian & Shin 2010 `10.1016/j.jfi.2008.12.002`). Step 2 agent+env: icon-four-check PASS 5/5 (profiles exist, Icon rows present, PNGs at `agent_images/icons/finance-{stem}.png`, mapping rows #99–#103 in `design.md`); AGENT_POOL three-stage match outcome = `reuse` for all 5 archetypes; environment/diversity/communication PASS. Step 3 config: 16 YAMLs parse; `# Source:` traceability present; §9-authority PASS. Step 4 impl: py_compile PASS 16 files; import smoke PASS all 4 variants; no-defaults PASS; RuleLLM dual-section PASS; `_RAG_FALLBACK` PASS; `explain.md`/`analysis.md` bidirectional PASS. Steps 5-10: Rule 5-round e2e via `GeneralSimulator.setup()+run()+shutdown()` completes with 9 actors and no exception; LLM/RuleLLM/Rag setup-only smokes complete cleanly. Round 2 Closeout: no regression detected. Status confirmed `released`.
- 2026-07-21  Polish Round 1 full audit + Closeout. Preflight: 4 variant subdirs (Rule/LLM/RuleLLM/Rag) under `examples/GFC2008/`, 4 config folders under `configs/GFC2008/`, target file present at `finance-gfc-2008.md`. Step 0 (target-file gate, Case A): §11 structural gates PASS — 10 top-level sections in canonical order, 5 §4 theory anchors each with five-row table, §5 F1–F4 stylized facts with acceptance metrics, §7 five agent rows, §8 four subsections, §9 eleven parameters, §10.1 four `Yes` variants; Status upgraded `draft → locked → released`. Step 1 (research): DOI-resolution PASS on 5 target §4 theory anchors (Keys et al. 2010 `10.1162/qjec.2010.125.1.307`; Bolton et al. 2012 `10.1111/j.1540-6261.2011.01708.x`; Brunnermeier & Pedersen 2009 `10.1093/rfs/hhn098`; Griffin & Xu 2009 `10.1093/rfs/hhp003`; Adrian & Shin 2010 `10.1016/j.jfi.2008.12.002`); six-field completeness PASS; bidirectional target-anchor coverage PASS (5↔5). Step 2 (agent+env): AGENT_POOL three-stage match = `reuse` for all 5 (mbs-originator, rating-agency, leveraged-investor, distressed-buyer, regulator); icon-four-check initial FAIL — 5 PNGs missing + 5 design.md mapping rows missing; REPAIR: generated 5 PNGs via ImageGen, added mapping rows #99–#103 to `agent_images/design.md`; post-repair icon-four-check PASS 5/5; environment §3/§5/§7 structural PASS. Step 3 (config): YAML parse PASS 16 files; `# Source:` traceability present; §9 authority PASS; variant-folder presence PASS (4/4). Step 4 (impl): py_compile PASS all variants; import smoke PASS all variants; no-defaults PASS; RuleLLM dual-section PASS; `_RAG_FALLBACK` PASS; `explain.md` §2 bidirectional PASS; `analysis.md` §2 bidirectional PASS. Steps 5-10 (review+smoke): Rule 5-round end-to-end via `GeneralSimulator.setup()+run()+shutdown()` with `total_rounds=5` completes cleanly (9 actors: 1 market + 8 investors across 5 archetypes, no uncaught exception); LLM/RuleLLM/Rag setup-only smokes PASS. Status walked `locked → released` at Closeout.
...
|--------------------|------------------------------|---------------------------------------------|---------------|-------------------------------------|--------------------------------------------------------------------------------|---------------------------------------------------|
| mbs-originator     | securitization desk / bank   | Fee-income / Originate-to-distribute (§4.1) | Destabilising | position, origination_rate          | Exists to distribute securitized inventory at a constant rate.                 | examples/AGENT_POOL/finance/mbs-originator.md     |
| rating-agency      | credit rating agency         | Rating inflation / Issuer-pays (§4.2)       | Destabilising | price, fundamental, overrating_bias | Exists to buy against an inflated fundamental until price is well below it.    | examples/AGENT_POOL/finance/rating-agency.md      |
| leveraged-investor | hedge fund / investment bank | Leverage cycle / Margin spiral (§4.3)       | Destabilising | price, deviation, position          | Exists to fire-sell half of remaining position when deviation crosses trigger. | examples/AGENT_POOL/finance/leveraged-investor.md |
| distressed-buyer   | distressed / vulture fund    | Distressed-capital floor (§4.4)             | Stabilising   | price, deviation, cash              | Exists to buy deep-discount inventory once deviation crosses discount level.   | examples/AGENT_POOL/finance/distressed-buyer.md   |
| regulator          | central bank / Treasury      | Lender-of-last-resort / Backstop (§4.5)     | Stabilising   | deviation, rng_state, rescue_size   | Exists to inject probabilistic large purchases at extreme deviation.           | examples/AGENT_POOL/finance/regulator.md          |
