...
- **Mathematical Formulation**: Rational value response: value_demand = sign(F − P) × position_size when |(P − F) / F| > deviation_threshold. With deviation_threshold = 0.05, sentiment-driven mispricings smaller than 5% do not trigger fundamental correction; sentiment can persist within this band.
...
- **Calibration Implication**: `deviation_threshold` in §6 is set to 0.05 with candidate range 0.03–0.08 per Shleifer & Vishny (1997) limits-to-arbitrage cost structure; `position_size` in §6 is fixed at 300 shares per Graham (1949) non-speculative sizing discipline.

### 2.5 Noise-Trader Risk (Black; Barber & Odean)

- **Citation**: Black, F. (1986). "Noise." *Journal of Finance*, 41(3), 529–543. DOI: 10.1111/j.1540-6261.1986.tb04513.x. Also: Barber, B. M., & Odean, T. (2000). "Trading is hazardous to your wealth." *Journal of Finance*, 55(2), 773–806. DOI: 10.1111/j.1540-6261.2000.tb04002.x.
- **Core Insight**: Uninformed stochastic order flow creates liquidity and price-formation risk that prevents rational arbitrage from eliminating mispricing instantly. Black (1986) argues noise traders are the reason informed prices cannot fully converge to fundamental value in finite time; Barber & Odean (2000) document that retail order flow is empirically uncorrelated with subsequent fundamentals, matching the theoretical noise-trader signature.
- **Mathematical Formulation**: `trade ~ Bernoulli(p_trade)`, direction uniformly drawn from {buy, sell} conditional on activation; quantity `Q ~ Uniform(min_order, max_order)`. This is the operational form of Black's aggregate uninformed order flow.
- **Empirical Evidence**: Black (1986) estimates noise traders account for 20–40% of daily volume in mature equity markets; Barber & Odean (2000) find average retail portfolio turnover of ~75%/year, consistent with per-round trade probabilities in the 0.10–0.40 range for scenarios calibrated to behavioral-bias episodes. Retail volume negatively predicts subsequent returns, confirming the uninformed direction.
- **Relevance to Investor Taxonomy**: NoiseTrader implements this channel with `trade_probability = 0.30` and `Q ~ Uniform(100, 500)`. Its role is not to test a hypothesis but to prevent the price path from being deterministically pinned by the two biased channels and the two rational stabilisers, so cross-run statistical comparison of bias episodes is meaningful.
- **Calibration Implication**: `trade_probability` in §6 is set to 0.30 with candidate range 0.10–0.40 per Black (1986) volume-share estimate; `min_order` / `max_order` in §6 (100/500 shares) matches the retail-lot magnitude used in Black-noise calibrations across the released sibling scenarios (AssetBubble, AsianFinancialCrisis).
...
**Lesson for Simulation**: The COVID crash demonstrates that when both availability bias channels (recency and media) are active simultaneously, the combined effect (RecentEventOverweighter + MediaInfluencedTrader both selling) can produce a temporary overreaction episode even in the absence of permanent fundamental damage.
