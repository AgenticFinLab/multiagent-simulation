#!/usr/bin/env python
"""OverconfidenceBias Simulation Analysis

Analyze the OverconfidenceBias simulation results.

Usage:
    python examples/OverconfidenceBias/Rule/analysis.py \
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("OverconfidenceBias Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "overconfidencebias_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze OverconfidenceBias simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/OverconfidenceBias/Rule/simulation.yml")
