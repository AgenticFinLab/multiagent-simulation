#!/usr/bin/env python
"""LossAversion Simulation Analysis

Analyze the LossAversion simulation results.

Usage:
    python examples/LossAversion/Rule/analysis.py \
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("LossAversion Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "lossaversion_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze LossAversion simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/LossAversion/Rule/simulation.yml")
