#!/usr/bin/env python
"""SVBBankRun Simulation Analysis

Analyze the SVBBankRun simulation results.

Usage:
    python examples/SVBBankRun/Rule/analysis.py \
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("SVBBankRun Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "svbbankrun_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze SVBBankRun simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/SVBBankRun/Rule/simulation.yml")
