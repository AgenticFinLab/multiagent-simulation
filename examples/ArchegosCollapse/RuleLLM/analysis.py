...
def main() -> None:
    """Run ArchegosCollapse RuleLLM analysis: metrics + rule-adherence report.

    Implements analysis-bases.md §2 core metrics plus RuleLLM-specific
    rule-adherence analysis (Dimension 2). Output written to
    EXPERIMENT/ArchegosCollapse/RuleLLM/records/analysis/.
    """
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        default="configs/ArchegosCollapse/RuleLLM/simulation.yml",
    )
    ...
    fig.suptitle(
        "ArchegosCollapse RuleLLM Simulation Analysis", fontsize=14, fontweight="bold"
    )
    ...
    # Rule-adherence bar chart (replaces generic return distribution)
    if rule_adherence:
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(analysis_path, "archegsoscollapse_rulellm_analysis.png"), dpi=150
    )
    plt.close()
