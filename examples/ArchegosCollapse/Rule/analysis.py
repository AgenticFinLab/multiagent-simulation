#!/usr/bin/env python
"""ArchegosCollapse Simulation Analysis

Analyze the ArchegosCollapse simulation results.
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("ArchegosCollapse Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "archegsoscollapse_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze ArchegosCollapse simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/ArchegosCollapse/Rule/simulation.yml")
