...
| Status        | locked                                                                                                                                                                                                                                                                              |
| CHANGELOG     | 2026-07-03  Produced by define-simulation-scenario-skill.md v1.2.0 under polish-simulation-pipeline.md Step 0 Case B (Status: draft). Source of §2–§10 content: existing downstream artefacts under `examples/StatusQuoBias/` (simulation-bases.md, analysis-bases.md, Rule/players.py, configs/StatusQuoBias/Rule/players.yml). / 2026-07-03  Polish target-file gate: structural validation green (10/10 sections, 5 DOIs across §4, 5 stylized facts, 5 agents in §7, 4 variants in §10.1), Status upgraded draft → locked. |
...
| F2 | Active rebalance order flow is visibly present once deviation crosses the rational threshold.                             | Under `|deviation| > rebalance_threshold`, `ActiveRebalancer` total absolute quantity accounts for ≥ 25% of biased-group total quantity across the run.             | `compute_active_rebalance_volume` (analysis-bases.md §2.3).                                                                        |
...
| F4 | Momentum flow is observable but bounded.                                                                                  | Total `MomentumTrader` absolute quantity accounts for a distinct nonzero share of order flow (≥ 5%) under `|deviation| > entry_threshold`.                          | `compute_momentum_offset` (analysis-bases.md §2.5).                                                                                |
| F5 | Price-fundamental deviation remains bounded and finite for the full run.                                                  | `max(|deviation|)` finite and price path strictly positive; standard deviation of round-level deviation between 0.02 and 0.30.                                     | `compute_price_deviation` (analysis-bases.md §2.6).                                                                                |

## §6 Historical / Empirical Anchors

| Case                                                | Time                | Correspondence to model                                                                                                                                                                                                                                                                                                                       |
|-----------------------------------------------------|---------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 401(k) automatic-enrollment default                 | 1998–1999 cohort    | Participation rose from ~49% to ~86% after opt-out default (Madrian and Shea 2001, 10.1162/003355301753265543); maps to `DefaultFollower` holding until `|deviation| > active_deviation`.                                                                                                                                                    |
...
| Intermediate-horizon momentum anomaly               | 1965–1989 US sample | 3-to-12 month winners outperformed losers by ~1%/month (Jegadeesh and Titman 1993, 10.1111/j.1540-6261.1993.tb04702.x); maps to `MomentumTrader` sign-following once `|deviation| > entry_threshold`.                                                                                                                                        |
...
|--------------------------|---------------------|-----------:|--------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|--------------------------------------------------|
| Inertial holder          | `InertialHolder`    | 3          | Destabilising (inaction) | On `|deviation| > change_threshold` buys undervalued / sells overvalued exposure with `Q = max(1, int(base_size × |deviation| / change_threshold × (1 − inertia_strength + 0.1)))`; otherwise holds.                             | `deviation`, `price`, `cash`, `position` | Reacts to current-round deviation only.          |
| Default follower         | `DefaultFollower`   | 3          | Destabilising (inaction) | On `|deviation| > active_deviation` trades with `Q = max(1, int(base_size × |deviation| / active_deviation × max(default_weight, 0.1)))`; otherwise holds the passive default.                                                        | `deviation`, `price`, `cash`, `position` | Reacts to current-round deviation only.          |
| Active rebalancer        | `ActiveRebalancer`  | 2          | Stabilising        | On `|deviation| > rebalance_threshold` buys below and sells above fundamental with `Q = max(1, int(position_size × |deviation| / rebalance_threshold))`.                                                                                | `deviation`, `price`, `cash`, `position` | Reacts to current-round deviation only.          |
| Momentum trader          | `MomentumTrader`    | 2          | Destabilising (trend) | On `|deviation| > entry_threshold` sign-follows deviation with `Q = max(1, int(position_size × |deviation| / entry_threshold))`; capped by cash/position.                                                                              | `deviation`, `price`, `cash`, `position` | Reacts to current-round deviation only.          |
...
| `inertialholder.extras.change_threshold`           | Status-quo action threshold `|deviation| > θ`                      | `0.30`      | simulation-bases.md §4.1; Samuelson and Zeckhauser (1988), 10.1007/BF00055564                                                |
...
| `defaultfollower.extras.active_deviation`          | Default-follower action threshold `|deviation| > θ`                | `0.15`      | simulation-bases.md §4.2; Madrian and Shea (2001), 10.1162/003355301753265543                                                |
...
| `activerebalancer.extras.rebalance_threshold`      | Active trading threshold `|deviation| > θ`                         | `0.05`      | simulation-bases.md §4.3; Markowitz (1952), 10.1111/j.1540-6261.1952.tb01525.x                                                |
...
| `momentumtrader.extras.entry_threshold`            | Momentum activation threshold `|deviation| > θ`                    | `0.01`      | simulation-bases.md §4.4; Jegadeesh and Titman (1993), 10.1111/j.1540-6261.1993.tb04702.x                                    |
