#!/usr/bin/env python
"""StatusQuoBias Rag analysis with retrieval diagnostics."""
...
def load_simulation_data(config: Dict[str, Any]) -> Dict[str, Any]:
    """Load StatusQuoBias RAG data and recorded `rag_context` fields."""
...
def analyze_rag_knowledge_effect(
    ...
    if total_rounds == 0:
        raise ValueError("No recorded RAG contexts found for StatusQuoBias Rag analysis")
...
def main() -> Dict[str, Any]:
    """Run StatusQuoBias Rag analysis and write `rag_stats.json`."""
    parser = argparse.ArgumentParser(description="Analyze StatusQuoBias Rag results")
    ...
    summary = analyze_statusquobias(data, config, str(output_dir))
