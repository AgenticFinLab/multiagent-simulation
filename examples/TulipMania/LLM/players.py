"""TulipMania LLM Simulation

LLM-driven variant of TulipMania.

Design:
    - Market: Rule-based coordinator (identical to TulipMania.Rule.Market).
    - Investors: LLM-powered with system prompts encoding persona + decision rules.
...
from .prompts import (
    LLM_TREND_CHASER_SYS,
    LLM_SOCIAL_PROOF_FOLLOWER_SYS,
    LLM_INTRINSIC_VALUE_TRADER_SYS,
    LLM_EARLY_EXIT_TRADER_SYS,
...
class LLMInvestor(GeneralPlayer):
    """Base class for LLM-driven TulipMania investors."""
...
class LLMTrendChaser(LLMInvestor):
    """LLM trend chaser buying assets purely because prices are rising."""

    _system_prompt = LLM_TREND_CHASER_SYS


class LLMSocialProofFollower(LLMInvestor):
    """LLM social proof follower joining speculative positions due to crowd behavior."""

    _system_prompt = LLM_SOCIAL_PROOF_FOLLOWER_SYS


class LLMIntrinsicValueTrader(LLMInvestor):
    """LLM intrinsic value trader selling when price far exceeds use value."""

    _system_prompt = LLM_INTRINSIC_VALUE_TRADER_SYS


class LLMEarlyExitTrader(LLMInvestor):
    """LLM early exit trader recognizing speculative excess and exiting early."""

    _system_prompt = LLM_EARLY_EXIT_TRADER_SYS
