"""TulipMania Rag Simulation

RAG-augmented hybrid Rule+LLM variant of TulipMania.
...
from .prompts import (
    RAGLLM_TREND_CHASER_SYS,
    RAGLLM_SOCIAL_PROOF_FOLLOWER_SYS,
    RAGLLM_INTRINSIC_VALUE_TRADER_SYS,
    RAGLLM_EARLY_EXIT_TRADER_SYS,
...
class RagLLMInvestor(GeneralPlayer):
    """Base class for RAG-augmented TulipMania investors."""
    ...
    def _build_prompt(self) -> str:
        ...
        if rag_store and rag_store.is_built():
            ...
            query = KnowledgeQuery(
                text=(
                    f"speculative bubble mania: deviation={deviation * 100:+.2f}%, "
...
class RagLLMTrendChaser(RagLLMInvestor):
    """RAG-augmented trend chaser buying assets purely because prices are rising."""

    _system_prompt = RAGLLM_TREND_CHASER_SYS


class RagLLMSocialProofFollower(RagLLMInvestor):
    """RAG-augmented social proof follower joining speculative positions due to crowd behavior."""

    _system_prompt = RAGLLM_SOCIAL_PROOF_FOLLOWER_SYS


class RagLLMIntrinsicValueTrader(RagLLMInvestor):
    """RAG-augmented intrinsic value trader selling when price far exceeds use value."""

    _system_prompt = RAGLLM_INTRINSIC_VALUE_TRADER_SYS


class RagLLMEarlyExitTrader(RagLLMInvestor):
    """RAG-augmented early exit trader recognizing speculative excess and exiting early."""

    _system_prompt = RAGLLM_EARLY_EXIT_TRADER_SYS
