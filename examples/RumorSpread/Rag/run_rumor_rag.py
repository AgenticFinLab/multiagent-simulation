#!/usr/bin/env python3
"""RumorSpreadRag Simulation Runner

Run RAG-augmented Rule+LLM rumor spread simulation.
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run RumorSpreadRag Simulation")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
    ...
    print("\n" + "=" * 60)
    print("RumorSpreadRag Simulation")
    print("=" * 60)
    print("Phenomenon: Rumor Propagation with RAG-Augmented LLM Agents")
    print("Theory:     Allport & Postman + Explicit Rules + RAG Knowledge")
    print("Agents:     Gullible Spreader, Distorting Relayer,")
    print("            Skeptical Evaluator, Fact Checker,")
    print("            Uninformed Bystander  (all Rule+LLM+RAG)")
    ...
    try:
        ...
        sim_results = await simulator.run()
        print("\n" + "=" * 60)
