...
Design:
    - Market coordinator: identical rule-based price dynamics as VolatilityClustering
...
meaningful comparison across three variants:
    ...
    VolatilityClusteringLLM     - pure LLM (persona only)
    VolatilityClusteringRuleLLM - hybrid (persona + explicit rules in prompt)
...
1. **Via Streamlit Web UI (Recommended):**
   ...
   ```
   Then select "VolatilityClusteringRuleLLM" from the scenario dropdown.
...
from .prompts import (
    RULELLM_FUNDAMENTALIST_SYS,
    RULELLM_TREND_FOLLOWER_SYS,
    RULELLM_NOISE_TRADER_SYS,
    RULELLM_SLOW_ADAPTER_SYS,
    RULELLM_VOLATILITY_TRADER_SYS,
)

logger = logging.getLogger("VolatilityClusteringRuleLLM")


# =============================================================================
# Market - Rule-Based Coordinator (identical to VolatilityClustering.Market)
# =============================================================================
...
class Market(GeneralPlayer):
    ...
    Price model (rule-based, unchanged from VolatilityClustering):
        Price impact increases when liquidity is low (volatility amplification mechanism).
...
class RuleLLMInvestor(GeneralPlayer):
    """Base class for hybrid Rule+LLM volatility-clustering investors."""
