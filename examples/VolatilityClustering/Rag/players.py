"""VolatilityClusteringRag — RAG-augmented Rule+LLM Volatility Clustering Simulation
...
This extends the three-variant comparison:
    ...
    VolatilityClusteringRuleLLM — LLM with rules in prompt (no external knowledge)
    VolatilityClusteringRag     — LLM with rules in prompt + RAG knowledge retrieval
...
1. **Via Streamlit Web UI (Recommended):**
   ...
   ```
   Then select "VolatilityClusteringRag" from the scenario dropdown.

2. **Command Line:**

   ```bash
   python examples/VolatilityClustering/Rag/run_flash_crash_ragllm.py \
...
class RagLLMInvestor(GeneralPlayer):
    """
    Base class for RAG-augmented Rule+LLM volatility clustering investors.
