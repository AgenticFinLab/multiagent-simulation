#!/usr/bin/env python
"""DispositionEffectRuleLLM Simulation Runner

Run hybrid Rule+LLM investor simulation for disposition effect.
Uses ByteDance Doubao API via lmbase.

Usage:
    python examples/DispositionEffect/RuleLLM/run_disposition_rulellm.py -c configs/DispositionEffect/RuleLLM/simulation.yml
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run DispositionEffectRuleLLM Simulation"
    )
    parser.add_argument(
        "-c",
        "--config",
        type=str,
        default="configs/DispositionEffect/RuleLLM/simulation.yml",
    ...
    print("\n" + "=" * 60)
    print("DispositionEffectRuleLLM Simulation (ByteDance Doubao)")
    print("=" * 60)
