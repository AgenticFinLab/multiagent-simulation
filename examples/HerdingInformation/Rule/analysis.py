#!/usr/bin/env python
"""HerdingInformation Simulation Analysis

Analyze the HerdingInformation simulation results.

Usage:
    python examples/HerdingInformation/Rule/analysis.py \
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("HerdingInformation Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "herdinginformation_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze HerdingInformation simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/HerdingInformation/Rule/simulation.yml")
