#!/usr/bin/env python
"""Volmageddon Simulation Analysis

Analyze the Volmageddon simulation results.

Usage:
    python examples/Volmageddon/Rule/analysis.py \
...
def create_visualizations(data: dict, output_path: str) -> None:
    ...
    fig.suptitle("Volmageddon Simulation Analysis", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "volmageddon_analysis.png"), dpi=150)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Analyze Volmageddon simulation results")
    parser.add_argument("-c", "--config", type=str, default="configs/Volmageddon/Rule/simulation.yml")
