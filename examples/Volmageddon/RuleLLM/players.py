"""Volmageddon RuleLLM Simulation
...
Design:
    ...
    - Investors: Hybrid rule+LLM with system prompts encoding persona + rules,
      combined with rule-based fallback logic.
...
from .prompts import (
    RULELLM_SHORT_VOL_TRADER_SYS,
    RULELLM_VOL_ETN_MANAGER_SYS,
    RULELLM_LONG_VOL_HEDGER_SYS,
    RULELLM_VOL_ARBITRAGEUR_SYS,
    RULELLM_EQUITY_TRADER_SYS,
)
from ..Rule.players import Market  # noqa: F401 — re-exported

logger = logging.getLogger("Volmageddon.RuleLLM")


class RuleLLMInvestor(GeneralPlayer):
    """Base class for RuleLLM-driven Volmageddon investors."""
...
class RuleLLMShortVolTrader(RuleLLMInvestor):
    """RuleLLM-driven short volatility trader selling VIX futures/ETNs for carry."""

    _system_prompt = RULELLM_SHORT_VOL_TRADER_SYS


class RuleLLMVolETNManager(RuleLLMInvestor):
    """RuleLLM-driven inverse VIX ETN manager with procyclical rebalancing mechanics."""

    _system_prompt = RULELLM_VOL_ETN_MANAGER_SYS


class RuleLLMLongVolHedger(RuleLLMInvestor):
    """RuleLLM-driven long volatility hedger holding VIX as portfolio insurance."""

    _system_prompt = RULELLM_LONG_VOL_HEDGER_SYS


class RuleLLMVolArbitrageur(RuleLLMInvestor):
    """RuleLLM-driven volatility arbitrageur trading VIX term structure dislocations."""

    _system_prompt = RULELLM_VOL_ARBITRAGEUR_SYS


class RuleLLMEquityTrader(RuleLLMInvestor):
    """RuleLLM-driven equity trader navigating volatility spikes and fundamental dislocations."""

    _system_prompt = RULELLM_EQUITY_TRADER_SYS
