    ...
    for agent_id, records in agent_records.items():
        ...
        if total_rag_rounds > 0:
            rag_stats[agent_id] = {
                ...
                "retrieval_success_rate": float(success_rounds / total_rag_rounds),
            }
    return rag_stats
...
def main() -> None:
    """Run CarryTradeUnwind Rag analysis: metrics + RAG knowledge-effect report.

    Implements analysis-bases.md §2 core metrics plus Rag-specific
    knowledge-retrieval analysis. Output written to
    EXPERIMENT/CarryTradeUnwind/Rag/records/analysis/.
    """
    ...
    fig.suptitle(
        "CarryTradeUnwind Rag Simulation Analysis", fontsize=14, fontweight="bold"
    )
    ...
    axes[0, 0].set_title("FX Rate vs PPP Fundamental (Rag)")
    axes[0, 0].legend()
    ...
    if len(fundamentals) > 0 and fundamentals[0] > 0:
        ...
        axes[0, 1].axhline(
            y=-5, color="red", linestyle=":", alpha=0.7, label="-5% threshold"
        )
    ...
    if rag_stats:
        ...
        success_rates = [rag_stats[a]["retrieval_success_rate"] for a in agent_ids]
        colors = ["teal" if r >= 0.70 else "coral" for r in success_rates]
        bars = axes[1, 1].bar(
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(analysis_path, "carrytradeunwind_rag_analysis.png"), dpi=150
    )
    plt.close()
