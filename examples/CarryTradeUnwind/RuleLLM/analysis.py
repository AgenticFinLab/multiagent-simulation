...
def main() -> None:
    """Run CarryTradeUnwind RuleLLM analysis: metrics + rule-adherence report.

    Implements analysis-bases.md §2 core metrics plus RuleLLM-specific
    rule-adherence analysis. Output written to
    EXPERIMENT/CarryTradeUnwind/RuleLLM/records/analysis/.
    """
    ...
    fig.suptitle(
        "CarryTradeUnwind RuleLLM Simulation Analysis", fontsize=14, fontweight="bold"
    )
    ...
    axes[0, 0].set_title("FX Rate vs PPP Fundamental (RuleLLM)")
    axes[0, 0].legend()
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(analysis_path, "carrytradeunwind_rulellm_analysis.png"), dpi=150
    )
    plt.close()
