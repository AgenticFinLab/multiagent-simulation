#!/usr/bin/env python
"""EquityPremiumRuleLLM Simulation Runner

Run hybrid Rule+LLM equity premium investor simulation.
Each LLM agent follows explicit quantitative rules (embedded in system prompt)
alongside a rich persona description.

Usage:
    python examples/EquityPremium/RuleLLM/run_equity_premium_rulellm.py -c configs/EquityPremium/RuleLLM/simulation.yml
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run EquityPremiumRuleLLM Simulation")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
    ...
    print("\n" + "=" * 60)
    print("EquityPremiumRuleLLM Simulation")
    print("=" * 60)
    print("Phenomenon: EquityPremium with Rule-Guided LLM Decision-Making")
