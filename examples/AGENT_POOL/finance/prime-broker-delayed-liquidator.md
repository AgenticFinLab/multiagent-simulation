...
- Given deviation = -0.16 (below `liquidation_threshold` of -0.15), agent must emit a sell order with quantity = position * 0.35.
- Given deviation = -0.12 (above `liquidation_threshold` of -0.15), agent must hold with zero quantity even though the first-mover archetype would fire.
- Given position is zero (collateral exhausted), agent must hold regardless of deviation magnitude.
- Given a sell is triggered, agent must apply `price_penalty` (0.97) to the execution price, receiving worse execution than the first-mover.
- Given any prerequisite signal is missing or NaN, agent must hold and emit zero quantity.

#### Ablation Hooks

| Ablation name | Setting | Hypothesis tested | Expected direction | Metric |
|---------------|---------|-------------------|--------------------|--------|
| `threshold_strict` | `liquidation_threshold = -0.225` (50% deeper) | Fewer activations weaken this agent's cascade contribution | decrease | number of non-hold orders per episode |
| `no_penalty` | `price_penalty = 1.0` | Without execution haircut, first-mover advantage disappears | decrease | execution price gap between first-mover and delayed liquidator |
