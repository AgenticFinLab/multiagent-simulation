...
|---------------------------------------------|----------------------------------------------------------------------------------------------------------------|
| Fischhoff (1975) "knew-it-all-along" effect | `if abs(deviation) > 0.02: qty = min(800, int(abs(dev) × 5000 × hindsight_inflation × prediction_overweight))` |
...
| Theory Component                               | Implementation                                                                                 |
|------------------------------------------------|------------------------------------------------------------------------------------------------|
| Fischhoff & Beyth (1975) selective attribution | Same threshold as §4.1 (0.02); `success_attribution` and `failure_discount` modulate quantity  |
...
| Theory Component                                 | Implementation                                                                                     |
|--------------------------------------------------|----------------------------------------------------------------------------------------------------|
| Roese & Vohs (2012) process-oriented rationality | `if abs(deviation) > 0.05: qty = min(500, int(abs(dev) × 3000 × process_weight × outcome_weight))` |
...
| Quantity formula                         | `min(500, int(abs(dev) × 3000 × skepticism_level)) + position_size`                  |
