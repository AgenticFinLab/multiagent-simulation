...
RELEASE_ROOT = (
    PROJECT_ROOT / "execution/panic_1907/policy-realization-v0.1"
)
...
def test_panic_policy_realization_release_is_strictly_admitted() -> None:
    manifest = json.loads(
    ...
    assert manifest["coverage"]["registered_implementations"] == 34


def test_panic_policy_realization_is_reproducible_from_static_objects() -> None:
    released = json.loads(
        (RELEASE_ROOT / "policy-realization.json").read_text(encoding="utf-8")
    )

    assert released == build_panic_policy_realization_document(
        project_root=PROJECT_ROOT
    )


def test_panic_policy_realization_manifest_and_checksums_close() -> None:
    manifest = json.loads(
        (RELEASE_ROOT / "manifest.json").read_text(encoding="utf-8")
    )
