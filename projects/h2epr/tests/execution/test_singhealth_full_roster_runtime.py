...
from h2epr.scenarios.singhealth_data_breach.full_roster_v0_1.executable_admission import (
    ExecutableAdmission,
)
from h2epr.scenarios.singhealth_data_breach.full_roster_v0_1.runtime_execution import (
...
def _in_memory_admission() -> ExecutableAdmission:
    bundle = build_singhealth_runtime_bundle_document(project_root=PROJECT_ROOT)
...
def test_independent_full_roster_materializations_are_identical(tmp_path: Path) -> None:
    ...
    first = materialize_singhealth_run(admission, tmp_path / "first")
    second = materialize_singhealth_run(admission, tmp_path / "second")
    ...
    assert coverage["actors_operated"] == 13
    assert coverage["actor_capability_bindings"] == 13
    assert coverage["commitments_evaluated"] == 41
    assert coverage["scenario_policies_exercised"] == 9
    assert coverage["lifecycle_families_realized"] == 11
    assert coverage["record_counts"]["action_intent"] == 41
    assert coverage["record_counts"]["participant_decision"] == 41

    actors = {
        row["actor_id"]
        for row in admission.runtime_bundle_document["actor_registry"]
    }
    context_deliveries = {
        row["payload"]["recipient_id"]
        for row in first.simulation_trace
        if row["record_type"] == "message_disposition"
        and row["payload"]["status"] == "delivered"
        and row["payload"]["recipient_id"] not in actors
    }
    assert context_deliveries
    assert context_deliveries <= {
        "institution.0616.csa",
        "institution.0616.moh",
        "process.0616.notification",
    }
    assert {
...
def test_generated_epg_resolves_only_to_sealed_trace(tmp_path: Path) -> None:
    artifacts = materialize_singhealth_run(
