...
def project_copy(tmp_path_factory: pytest.TempPathFactory) -> Path:
    root = tmp_path_factory.mktemp("singhealth-executable-admission") / "h2epr"
    shutil.copytree(PROJECT_ROOT, root)
    return root


def test_singhealth_executable_release_is_strictly_admitted() -> None:
    manifest = json.loads(
...
def test_bundle_drift_fails_even_when_reference_hashes_are_updated(
    project_copy: Path,
) -> None:
    release = (
        project_copy
...
def test_duplicate_json_and_expected_source_drift_fail_closed(
    ...
    assert integrity_raised.value.code is (
