...
from h2epr.execution import LifecycleRecord, LifecycleRuleError
from h2epr.scenarios.singhealth_data_breach.full_roster_v0_1 import (
    build_singhealth_policy_catalog,
)
from h2epr.scenarios.singhealth_data_breach.full_roster_v0_1.lifecycle_rules import (
    LIFECYCLE_RULES,
)
...
def test_lifecycle_registry_closes_catalog_and_participant_references() -> None:
    ...
    with pytest.raises(KeyError, match="unknown_lifecycle_rule"):
        lifecycle_rule("h2epr.lifecycle.0616.unknown")


def test_every_lifecycle_state_is_reachable_from_an_initial_state() -> None:
    for rule in LIFECYCLE_RULES:
...
def test_every_lifecycle_transition_is_deterministic_and_versioned() -> None:
    for rule in LIFECYCLE_RULES:
...
def test_malformed_lifecycle_records_fail_before_reduction() -> None:
    ...
    with pytest.raises(LifecycleRuleError, match="lifecycle_record_invalid"):
        rule.transition(
            replace(record, lifecycle_id="lifecycle.0616.wrong"),
