"""Deterministic assembly of the Note7 executable package and bundle."""
...
PACKAGE_ID = "h2epr.0481.full-roster-rule.v0_1"
PACKAGE_VERSION = "0.1.0"
RUNTIME_BUNDLE_ID = "h2epr.0481.rule-runtime-bundle.v0_1"
RUNTIME_BUNDLE_VERSION = "0.1.0"
RUN_PROFILE_ID = "h2epr.0481.run-profile.canonical.v0_1"
RUN_SEED = 481
POLICY_REALIZATION_ID = "h2epr.0481.policy-realization.v0_1"
...
_CAPABILITY_START_ANCHOR_INDEX = {
    "samsung_crisis_decision_interface": 1,
    "cpsc_recall_decision_interface": 1,
    "caac_warning_decision_interface": 2,
    "us_dot_emergency_order_decision_interface": 2,
    "samsung_regional_implementation_units": 2,
    "carrier_and_retail_remedy_outlets": 2,
    "note7_owners_and_prospective_consumers": 1,
    "air_transport_operators": 2,
}

_CAPABILITY_ACTIVATION_INPUTS = {
    "samsung_crisis_decision_interface": (
        "exo.0481.bounded-hazard-signal",
        "exo.0481.device-and-product-context",
        "exo.0481.product-flow-and-resource-opportunity",
    ),
    "cpsc_recall_decision_interface": (
        "exo.0481.bounded-hazard-signal",
        "exo.0481.institutional-authority-context",
        "exo.0481.recall-and-remedy-process-opportunity",
    ),
    "caac_warning_decision_interface": (
        "exo.0481.institutional-authority-context",
        "exo.0481.transport-process-opportunity",
    ),
    "us_dot_emergency_order_decision_interface": (
        "exo.0481.institutional-authority-context",
        "exo.0481.transport-process-opportunity",
    ),
    ...
    "air_transport_operators": (
        "exo.0481.device-and-product-context",
        "exo.0481.transport-process-opportunity",
    ),
}

_PREFERRED_INTENT_BY_COMMITMENT = {
    "DC-SAM-1": "request_safety_investigation",
    "DC-SAM-2": "issue_product_flow_direction",
    "DC-SAM-3": "decide_production_posture",
    "DC-CPSC-1": "issue_consumer_warning",
    "DC-CPSC-2": "issue_recall_action",
    "DC-CPSC-3": "expand_recall_action",
    "DC-CAAC-1": "issue_transport_warning",
    "DC-CAAC-2": "qualify_transport_warning",
    "DC-DOT-1": "qualify_emergency_order",
    "DC-DOT-2": "issue_emergency_order",
    "situation_a": None,
}

_PREFERRED_POPULATION_INTENT = {
    ("samsung_regional_implementation_units", "situation_a"): (
        "coordinate_local_partner_response"
    ),
    ("samsung_regional_implementation_units", "situation_b"): (
        "propose_local_remedy"
    ),
    ("samsung_regional_implementation_units", "situation_c"): (
        "publish_local_safety_message"
    ),
    ("carrier_and_retail_remedy_outlets", "situation_a"): (
        "request_inventory_action"
    ),
    ("carrier_and_retail_remedy_outlets", "situation_b"): (
        "respond_to_remedy_request"
    ),
    ("carrier_and_retail_remedy_outlets", "situation_c"): (
        "set_local_product_posture"
    ),
    ("note7_owners_and_prospective_consumers", "situation_a"): (
        "choose_device_use_posture"
    ),
    ("note7_owners_and_prospective_consumers", "situation_b"): (
        "submit_incident_report"
    ),
    ("note7_owners_and_prospective_consumers", "situation_c"): (
        "request_exchange_or_refund"
    ),
    ("air_transport_operators", "situation_a"): "publish_operator_notice",
    ("air_transport_operators", "situation_b"): (
        "propose_carriage_denial_or_handling"
    ),
    ("air_transport_operators", "situation_c"): (
        "adopt_stricter_local_measure"
    ),
}


class Note7AssemblyError(ValueError):
    """The accepted Note7 parents cannot produce a closed bundle."""
...
def _project_root(supplied: str | Path | None) -> Path:
    ...
    if not root.is_dir() or not root.joinpath("src/h2epr").is_dir():
        raise Note7AssemblyError(
            "note7_assembly_project_root_invalid"
        )
    return root
...
def _baseline_private_state(
    ...
    *,
    ...
    for decision in policy.decisions.values():
        for state_id in decision.private_state_ids:
            ...
            if prior not in {None, value}:
                raise Note7AssemblyError(
                    f"note7_assembly_state_baseline_conflict:{state_id}"
                )
    ...
    if any(values[state_id] not in domains[state_id] for state_id in values):
        raise Note7AssemblyError(
            "note7_assembly_initial_state_outside_domain"
        )
...
def _private_state_sources(
    ...
    return {
        state_id: "participant_product_declared_default"
...
def _carrier_projections(
    ...
    for actor in actor_rows:
        ...
        carrier_id = f"h2epr.carrier.0481.{actor['actor_id']}.v0_1"
        carriers.append(
            {
...
def _observation_values_for_branch(
    ...
    *,
    ...
    ordered = sorted(
        decision.branches,
        key=lambda branch: branch.intent_id != preferred_intent_id,
    )
    ...
    *,
    ...
    for carrier in carriers:
        ...
        if len(available_ticks) < len(policy.decisions):
            raise Note7AssemblyError(
                f"note7_assembly_decision_horizon_short:{capability_id}"
            )
        ...
        for decision, evaluation_tick in zip(
            ...
            commitment_name = decision.commitment_id.rsplit(".", 1)[-1]
            preferred_name = _PREFERRED_POPULATION_INTENT.get(
                (capability_id, commitment_name)
            ) or _PREFERRED_INTENT_BY_COMMITMENT.get(commitment_name)
            if preferred_name is None:
                raise Note7AssemblyError(
                    "note7_assembly_preferred_intent_missing:"
                    f"{capability_id}:{commitment_name}"
                )
            preferred_intent_id = (
                f"h2epr.action.0481.{capability_id}.{preferred_name}"
            )
            observations, planned_branch = _observation_values_for_branch(
                decision,
                private_state=simulated_private,
                preferred_intent_id=preferred_intent_id,
            )
            ...
            if result.branch_id != planned_branch:
                raise Note7AssemblyError(
                    "note7_assembly_branch_projection_mismatch:"
    ...
    for lifecycle_id in sorted(set(LIFECYCLE_RULES_BY_ID) - covered):
        ...
        if candidate is None:
            raise Note7AssemblyError(
                f"note7_assembly_lifecycle_uncovered:{lifecycle_id}"
            )
...
def _direct_recipient(
    *,
    ...
    recipients = {
        "samsung_crisis_decision_interface": {
            "request_safety_investigation": None,
            ...
            "decide_production_posture": None,
            "publish_safety_message": (
                "actor.0481.unit.samsung-regional-singapore"
            ),
        },
        "cpsc_recall_decision_interface": {
            "issue_consumer_warning": "actor.0481.unit.consumer-primary",
            "request_incident_information": "actor.0481.interface.samsung-crisis",
            "request_remedy_information": "actor.0481.interface.samsung-crisis",
            "issue_recall_action": "actor.0481.interface.samsung-crisis",
            "expand_recall_action": "actor.0481.interface.samsung-crisis",
        },
        "caac_warning_decision_interface": {
            "request_transport_risk_information": (
                "actor.0481.interface.samsung-crisis"
            ),
            "issue_transport_warning": "actor.0481.unit.air-operator-primary",
            "qualify_transport_warning": "actor.0481.unit.air-operator-primary",
        },
        "us_dot_emergency_order_decision_interface": {
            "request_hazard_information": "actor.0481.interface.cpsc-recall",
            "qualify_emergency_order": "actor.0481.unit.air-operator-primary",
            "issue_emergency_order": "actor.0481.unit.air-operator-primary",
        },
        "samsung_regional_implementation_units": {
            "request_regional_clarification": "actor.0481.interface.samsung-crisis",
            "coordinate_local_partner_response": (
                "actor.0481.unit.outlet-singapore-channel"
            ),
            "propose_local_remedy": "actor.0481.unit.outlet-singapore-channel",
            "publish_local_safety_message": (
                "actor.0481.unit.outlet-singapore-channel"
            ),
        },
        "carrier_and_retail_remedy_outlets": {
            ...
            "set_local_product_posture": None,
            "publish_outlet_notice": "actor.0481.unit.consumer-primary",
            "request_inventory_action": (
                "actor.0481.unit.samsung-regional-singapore"
            ),
            "respond_to_remedy_request": "actor.0481.unit.consumer-primary",
        },
        "note7_owners_and_prospective_consumers": {
            "choose_device_use_posture": None,
            "submit_incident_report": "actor.0481.interface.samsung-crisis",
            "request_safety_information": "actor.0481.interface.cpsc-recall",
            "choose_purchase_posture": None,
            "request_exchange_or_refund": (
                "actor.0481.unit.outlet-singapore-channel"
            ),
        },
        "air_transport_operators": {
            "request_transport_clarification": "actor.0481.interface.caac-warning",
            "publish_operator_notice": None,
            "request_device_identification": None,
            "propose_carriage_denial_or_handling": None,
            "adopt_stricter_local_measure": None,
            "escalate_transport_ambiguity": "actor.0481.interface.us-dot-order",
        },
    }
    try:
        ...
        raise Note7AssemblyError(
            "note7_assembly_recipient_projection_missing:"
            f"{capability_id}:{suffix}"
        ) from exc


def _route_id(source_id: str, target_id: str) -> str:
    return f"route.0481.{source_id}.to.{target_id}"
...
def _scenario_policy_ids_for_action(
    *,
    ...
    if direct_recipient_id is not None:
        policy_ids.update({"POL-0481-INFO-01", "POL-0481-ROUTE-01"})
    if any(
        token in suffix
        for token in ("hazard", "incident", "investigat", "safety_information")
    ):
        policy_ids.add("POL-0481-HAZARD-01")
    if any(
        token in suffix
        for token in (
            "exchange",
        ...
        policy_ids.add("POL-0481-REMEDY-01")
    if any(
        token in suffix
        for token in (
            "inventory",
        ...
        policy_ids.add("POL-0481-PRODUCT-01")
    if any(
        token in suffix
        for token in (
            "warning",
        ...
        policy_ids.add("POL-0481-PUBLIC-ACTION-01")
    return sorted(policy_ids)


def _authority_context(
    actor: Mapping[str, Any], direct_recipient_id: str | None
) -> dict[str, Any]:
    access_ref = (
    ...
    parties = {actor["actor_id"], direct_recipient_id}
    if parties & {
        "actor.0481.interface.caac-warning",
        "actor.0481.interface.us-dot-order",
        "actor.0481.unit.air-operator-primary",
    }:
        relationship_ref = "opening.0481.relationship.transport-domains"
    elif parties & {"actor.0481.unit.consumer-primary"}:
        relationship_ref = "opening.0481.relationship.market-consumer"
    elif parties & {
        "actor.0481.unit.samsung-regional-singapore",
        "actor.0481.unit.outlet-singapore-channel",
    }:
        relationship_ref = "opening.0481.relationship.samsung-market"
    else:
        relationship_ref = "opening.0481.relationship.samsung-cpsc"
    target_by_capability = {
        "samsung_crisis_decision_interface": "asset.0481.product-flow",
        "cpsc_recall_decision_interface": "process.0481.recall",
        "caac_warning_decision_interface": "process.0481.transport-action",
        "us_dot_emergency_order_decision_interface": "process.0481.transport-action",
        "samsung_regional_implementation_units": "asset.0481.product-flow",
        "carrier_and_retail_remedy_outlets": "asset.0481.inventory-remedy",
        "note7_owners_and_prospective_consumers": (
            "asset.0481.device.associated-context"
        ),
        "air_transport_operators": "asset.0481.transport-encounter",
    }
    return {
        ...
        "resource_owner_id": actor["resource_owner_id"],
        "technical_target_id": target_by_capability[actor["capability_ids"][0]],
    }
            ...
            rows.append(
                {
                    ...
                    "authority_context": _authority_context(actor, recipient),
                    "direct_recipient_id": recipient,
...
def _participant_artifacts(placements: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for placement in placements.values():
        row = {
            ...
            "participant_policy_implementation_id": (
                f"h2epr.policy.0481.participant.{placement.capability_id}"
            ),
            "participant_policy_implementation_version": "0.1.0",
        ...
        if prior is not None and prior != row:
            raise Note7AssemblyError(
                "note7_assembly_participant_artifact_conflict"
            )
...
def _exogenous_inputs(
    ...
    release_anchor = {
        "exo.0481.bounded-hazard-signal": "modeled_start",
        "exo.0481.device-and-product-context": "modeled_start",
        "exo.0481.institutional-authority-context": "modeled_start",
        "exo.0481.product-flow-and-resource-opportunity": (
            "participant_response_start"
        ),
        "exo.0481.recall-and-remedy-process-opportunity": (
        ...
        "exo.0481.transport-process-opportunity": "core_horizon",
    }
    first_tick = {
...
def build_note7_runtime_bundle_document(
    *,
    project_root: str | Path | None = None,
    status: str = "accepted_runtime_bundle",
) -> dict[str, Any]:
    """Materialize the complete deterministic input to the Note7 runtime."""

    if status not in {"candidate", "accepted_runtime_bundle"}:
        raise Note7AssemblyError(
            "note7_runtime_bundle_status_invalid"
        )
    root = _project_root(project_root)
    configuration = _configuration(root)
    catalog = build_note7_policy_catalog(project_root=root)
    realization_admission = load_note7_policy_realization(
    ...
    if not realization_admission.accepted:
        raise Note7AssemblyError(
            "note7_assembly_policy_realization_not_accepted"
        )
    ...
    return {
        ...
        "compiler_inputs": {
            "compiler_implementation_id": (
                "h2epr.component.0481.trace-compiler"
            ),
...
def build_note7_executable_package_document(
    *,
    ...
    if status not in {"candidate", "accepted_executable_package"}:
        raise Note7AssemblyError(
            "note7_executable_package_status_invalid"
        )
    bundle = build_note7_runtime_bundle_document(
    ...
    return {
        ...
        "runtime_bundle_contract": {
            "format_identity": "h2epr.rule-runtime-bundle.v0_1",
            "builder_implementation_id": "h2epr.runtime.0481.bundle-builder",
