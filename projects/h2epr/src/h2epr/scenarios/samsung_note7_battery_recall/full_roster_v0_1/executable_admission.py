"""Fail-closed admission for the Note7 executable successor."""
...
from .runtime_components import Note7Environment
...
class ExecutableAdmissionCode(str, Enum):
    ...
    PREFLIGHT_INVALID = "NOTE7_EXECUTABLE_PREFLIGHT_INVALID"
    SOURCE_NOT_FOUND = "NOTE7_EXECUTABLE_SOURCE_NOT_FOUND"
    PATH_UNSAFE = "NOTE7_EXECUTABLE_PATH_UNSAFE"
    INTEGRITY_MISMATCH = "NOTE7_EXECUTABLE_INTEGRITY_MISMATCH"
    JSON_INVALID = "NOTE7_EXECUTABLE_JSON_INVALID"
    JSON_DUPLICATE_KEY = "NOTE7_EXECUTABLE_JSON_DUPLICATE_KEY"
    SCHEMA_INVALID = "NOTE7_EXECUTABLE_SCHEMA_INVALID"
    SCHEMA_VALIDATION_FAILED = (
        "NOTE7_EXECUTABLE_SCHEMA_VALIDATION_FAILED"
    )
    PACKAGE_IDENTITY_MISMATCH = (
        "NOTE7_EXECUTABLE_PACKAGE_IDENTITY_MISMATCH"
    )
    BUNDLE_IDENTITY_MISMATCH = (
        "NOTE7_EXECUTABLE_BUNDLE_IDENTITY_MISMATCH"
    )
    BUNDLE_MATERIALIZATION_MISMATCH = (
        "NOTE7_EXECUTABLE_BUNDLE_MATERIALIZATION_MISMATCH"
    )
    PACKAGE_MATERIALIZATION_MISMATCH = (
        "NOTE7_EXECUTABLE_PACKAGE_MATERIALIZATION_MISMATCH"
    )
    COMPONENT_UNRESOLVED = "NOTE7_EXECUTABLE_COMPONENT_UNRESOLVED"
    MASIM_BOUNDARY_MISMATCH = (
        "NOTE7_EXECUTABLE_MASIM_BOUNDARY_MISMATCH"
    )
    COVERAGE_MISMATCH = "NOTE7_EXECUTABLE_COVERAGE_MISMATCH"
...
def _verify_components(
    ...
    checks = Note7Environment(bundle).scenario_policy_checks()
...
def _verify_coverage(bundle: Mapping[str, Any]) -> dict[str, int]:
    ...
    if counts != {
        ...
        expected["actor_instances"] != 8
        or expected["actor_capability_bindings"] != 8
        or expected["intent_placements"] != 37
        or expected["decision_commitments"] != 22
        or expected["lifecycle_families"] != 12
...
def load_note7_executable_package(
    path: str | Path,
    *,
    ...
    first = build_note7_runtime_bundle_document(project_root=root)
    second = build_note7_runtime_bundle_document(project_root=root)
    ...
    expected_package = build_note7_executable_package_document(
