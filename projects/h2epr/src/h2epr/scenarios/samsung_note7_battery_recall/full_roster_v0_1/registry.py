"""Static Note7 Policy Realization implementation registry."""
...
if (
    ...
    raise ValueError("note7_implementation_registry_duplicate")

if set(_PARTICIPANT_POLICIES_BY_CAPABILITY) != set(
    DECISION_INTENT_INVENTORIES
):
    raise ValueError("note7_decision_intent_capability_coverage")
for capability_id, implementation in _PARTICIPANT_POLICIES_BY_CAPABILITY.items():
    ...
    if set(observed_by_name) != set(expected_by_name) or any(
        ...
        raise ValueError(
            f"note7_decision_intent_coverage:{capability_id}"
        )
...
if set(_CAPABILITIES_BY_LIFECYCLE) != {
    ...
    raise ValueError("note7_lifecycle_capability_coverage")
...
def implementation_versions() -> Mapping[str, str]:
    ...
    if len(versions) != len(implementations):
        raise ValueError("note7_implementation_registry_cross_kind_duplicate")
