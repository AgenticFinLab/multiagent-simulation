"""Deterministic construction of the accepted Panic Policy Realization."""
...
from .registry import lifecycle_rules, participant_policy, scenario_policies


def build_panic_policy_realization_document(
    *,
    project_root: str | Path | None = None,
    status: str = "accepted_policy_realization",
) -> dict[str, Any]:
    """Build the exact machine document from static reviewed implementations."""

    if status not in {"candidate", "accepted_policy_realization"}:
        raise ValueError("panic_policy_realization_status_invalid")
            ...
            state_rows.append(
                {
                    ...
                    "initialization": _state_initialization(
                        state_id,
                        parameter_names,
                    ),
                    "update_trigger_ids": list(update_triggers),
                }
            )
        decision_rows = [
            {
    ...
    for policy_id in catalog.selected_policy_ids:
        ...
        scenario_rows.append(
            {
                "policy_id": policy_id,
                "semantic_version": selected["version"],
...
def _project_root(supplied: str | Path | None) -> Path:
    ...
    if not root.is_dir() or not root.joinpath("src/h2epr").is_dir():
        raise ValueError("panic_policy_realization_project_root_invalid")
    return root


def _state_initialization(state_id: str, parameter_names: set[str]) -> str:
    reader_id = state_id.rsplit(".", 1)[-1]
    if reader_id in parameter_names or (
        reader_id == "response_profile_conflict_rule"
        and {"response_profile", "mixed_signal_rule"} <= parameter_names
    ):
        return "configuration_parameter"
    if reader_id.startswith("last_consumed_"):
...
def _state_update_triggers(
    ...
    if not observed:
        raise ValueError(f"panic_state_update_trigger_missing:{state_id}")
    return tuple(sorted(observed))


__all__ = ["build_panic_policy_realization_document"]
