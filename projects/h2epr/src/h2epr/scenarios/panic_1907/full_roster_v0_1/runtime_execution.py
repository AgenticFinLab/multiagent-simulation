"""Deterministic phased execution for the accepted Panic runtime bundle."""
...
from .scenario_rules import LIFECYCLE_POLICY


class PanicExecutionError(RuntimeError):
...
def _apply_delta(state: dict[str, Any], payload: Mapping[str, Any]) -> None:
    ...
    if canonical_sha256(target[field_name]) != canonical_sha256(payload["before"]):
        raise PanicExecutionError(
            "panic_replay_delta_before_mismatch:"
...
def build_panic_run_manifest(admission: ExecutableAdmission) -> dict[str, Any]:
    """Build the scientific run identity without operational coordinates."""

    bundle = admission.runtime_bundle_document
    preimage = {
        "format_identity": "h2epr.rule-run-manifest.v0_1",
        "run_id": "run.h2epr.0288.canonical.v0_1",
        "event_id": "H2EPR-0288",
...
class PanicRunArtifacts:
    """Scientific documents produced by one complete materialization."""
...
class PanicFullRosterEngine:
    ...
    def __init__(self, admission: ExecutableAdmission) -> None:
        if not admission.accepted or not admission.execution_eligible:
            raise PanicExecutionError("panic_execution_package_not_eligible")
        self.admission = admission
        self.bundle = _plain(admission.runtime_bundle_document)
        self.manifest = build_panic_run_manifest(admission)
        ...
        self.projector = PanicObservationProjector(self.bundle)
        self.executor = PanicParticipantExecutor()
        self.environment = PanicEnvironment(self.bundle)
        self.reducer = AuthoritativeReducer(
            self.bundle["initial_state"], PanicReducer(self.bundle).apply_batch
        )
        ...
        self.compiler = PanicTraceCompiler()
    ...
    async def setup(self, handles: Mapping[str, Any]) -> None:
        if tuple(sorted(handles)) != self.actor_ids:
            raise PanicExecutionError("panic_execution_actor_launch_mismatch")

    async def run_tick(
        self, logical_tick: int, barriers: tuple[str, ...]
    ) -> dict[str, Any]:
        if barriers != NAMED_BARRIERS:
            raise PanicExecutionError("panic_execution_barrier_order_mismatch")
        if logical_tick not in self._clock_by_tick or logical_tick in self.tick_results:
            raise PanicExecutionError("panic_execution_tick_invalid_or_repeated")
        ...
        for item in sorted(
            ...
            self._released_input_ids.append(item["input_id"])
            self.trace.append(
        ...
        if len(deliveries) != sum(len(rows) for rows in delivered_by_actor.values()):
            raise PanicExecutionError("panic_execution_delivery_consumption_mismatch")
        ...
        for rule in due_rules:
            ...
            if actual != rule["expected_outcome"]:
                raise PanicExecutionError(
                    "panic_execution_projected_outcome_mismatch:"
    ...
    def _append_completion_records(
        ...
        for object_id, item in open_objects:
            carried = LIFECYCLE_POLICY.carry_forward(
                ...
                next_event_id="event.0288.post_horizon",
            )
        ...
        if unresolved_ids or unresolved_recipients:
            raise PanicExecutionError("panic_execution_due_message_unresolved")
        self.trace.append(
            "completion",
            logical_tick,
            {
    ...
    def phase_dispatch(self, all_info_lists: list[list[dict[str, Any]]]) -> None:
        if all_info_lists:
            raise PanicExecutionError("panic_execution_legacy_dispatch_forbidden")

    def get_tick_result(self, logical_tick: int) -> dict[str, Any] | None:
        return copy.deepcopy(self.tick_results.get(logical_tick))

    def finalize(self) -> PanicRunArtifacts:
        if self._finalized:
            raise PanicExecutionError("panic_execution_already_finalized")
        if tuple(sorted(self.tick_results)) != self.logical_ticks:
            raise PanicExecutionError("panic_execution_tick_coverage_incomplete")
        ...
        if (
            ...
            raise PanicExecutionError("panic_execution_semantic_coverage_incomplete")
        final_state = self.reducer.state
        unresolved_ids, unresolved_recipients = self.transport.unresolved()
        if unresolved_ids or unresolved_recipients:
            raise PanicExecutionError("panic_execution_unresolved_transport")
...
class PanicFullRosterSimulator(PhasedSimulator):
    """Project binding for the public MASim phased simulator."""


class PanicFullRosterRunner(PhasedSimulationRunner):
    simulator_class = PanicFullRosterSimulator
...
async def _materialize(
    ...
    if operational_root.exists() and any(operational_root.iterdir()):
        raise FileExistsError("panic_operational_root_must_be_fresh")
    operational_root.mkdir(parents=True, exist_ok=True)
    engine = PanicFullRosterEngine(admission)
    config = SimulationConfig(
        setting={
            "name": "h2epr_0288_full_roster_rule",
    ...
    runner = PanicFullRosterRunner(config)
    results = await runner.execute()
    if len(results) != len(engine.logical_ticks):
        raise PanicExecutionError("panic_execution_result_tick_count_mismatch")
    return engine.finalize()


def materialize_panic_run(
    admission: ExecutableAdmission,
...
__all__ = [
    "PanicExecutionError",
    "PanicFullRosterEngine",
    "PanicFullRosterRunner",
    "PanicFullRosterSimulator",
    "PanicRunArtifacts",
    "build_panic_run_manifest",
