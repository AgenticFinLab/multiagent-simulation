"""Fail-closed admission for the SingHealth executable successor."""
...
from .runtime_components import SingHealthEnvironment
...
class ExecutableAdmissionCode(str, Enum):
    """Stable rejection classes for the executable boundary."""

    PREFLIGHT_INVALID = "SINGHEALTH_EXECUTABLE_PREFLIGHT_INVALID"
    SOURCE_NOT_FOUND = "SINGHEALTH_EXECUTABLE_SOURCE_NOT_FOUND"
    PATH_UNSAFE = "SINGHEALTH_EXECUTABLE_PATH_UNSAFE"
    INTEGRITY_MISMATCH = "SINGHEALTH_EXECUTABLE_INTEGRITY_MISMATCH"
    JSON_INVALID = "SINGHEALTH_EXECUTABLE_JSON_INVALID"
    JSON_DUPLICATE_KEY = "SINGHEALTH_EXECUTABLE_JSON_DUPLICATE_KEY"
    SCHEMA_INVALID = "SINGHEALTH_EXECUTABLE_SCHEMA_INVALID"
    SCHEMA_VALIDATION_FAILED = (
        "SINGHEALTH_EXECUTABLE_SCHEMA_VALIDATION_FAILED"
    )
    PACKAGE_IDENTITY_MISMATCH = (
        "SINGHEALTH_EXECUTABLE_PACKAGE_IDENTITY_MISMATCH"
    )
    BUNDLE_IDENTITY_MISMATCH = (
        "SINGHEALTH_EXECUTABLE_BUNDLE_IDENTITY_MISMATCH"
    )
    BUNDLE_MATERIALIZATION_MISMATCH = (
        "SINGHEALTH_EXECUTABLE_BUNDLE_MATERIALIZATION_MISMATCH"
    )
    PACKAGE_MATERIALIZATION_MISMATCH = (
        "SINGHEALTH_EXECUTABLE_PACKAGE_MATERIALIZATION_MISMATCH"
    )
    COMPONENT_UNRESOLVED = "SINGHEALTH_EXECUTABLE_COMPONENT_UNRESOLVED"
    MASIM_BOUNDARY_MISMATCH = (
        "SINGHEALTH_EXECUTABLE_MASIM_BOUNDARY_MISMATCH"
    )
    COVERAGE_MISMATCH = "SINGHEALTH_EXECUTABLE_COVERAGE_MISMATCH"
...
def _verify_coverage(bundle: Mapping[str, Any]) -> dict[str, int]:
    actors = tuple(bundle["actor_registry"])
    carriers = tuple(bundle["carrier_projections"])
    actions = tuple(bundle["action_registry"])
    observations = tuple(bundle["observation_rules"])
    lifecycles = tuple(bundle["lifecycle_registry"])
    routes = tuple(bundle["communication_routes"])
    placement_count = sum(
        len(row["capability_projections"]) for row in carriers
    )
    expected = bundle["coverage_expectations"]
    counts = {
        "actor_instances": len(actors),
        "actor_carriers": len(carriers),
        "actor_capability_bindings": placement_count,
        "action_bindings": len(actions),
        "decision_observation_rules": len(observations),
        "communication_routes": len(routes),
        "configured_route_records": len(
            bundle["initial_state"]["route_records"]
        ),
        "lifecycle_families": len(lifecycles),
    ...
    if counts != {
        ...
        "action_bindings": 74,
        "decision_observation_rules": 41,
        "communication_routes": 46,
        "configured_route_records": 8,
        "lifecycle_families": 11,
        "runtime_components": 9,
    } or (
        expected["actor_instances"] != 13
        or expected["actor_capability_bindings"] != 13
        or expected["intent_placements"] != 74
        or expected["decision_commitments"] != 41
        or expected["lifecycle_families"] != 11
        or expected["selected_policies"] != 9
    ):
        _fail(
    ...
    carrier_ids = {row["actor_id"] for row in carriers}
    ...
    if (
        actor_ids != carrier_ids
        or len(actor_ids) != len(actors)
        or len(route_ids) != len(routes)
        or len(lifecycle_ids) != len(lifecycles)
        or len(scenario_policy_ids) != 9
        or {
            policy_id
            for row in actions
        ...
        _fail(
            ExecutableAdmissionCode.COVERAGE_MISMATCH,
            pointer="/runtime_bundle",
            detail="registry_identity_or_policy_coverage",
        )
    if any(
        row["actor_id"] not in actor_ids
        or row["result_route_id"] not in route_ids
        or (
            row["direct_route_id"] is not None
            and row["direct_route_id"] not in route_ids
        )
        for row in actions
    ):
        _fail(
            ...
            detail="actor_or_route_reference_unresolved",
        )
    if (
        {
        ...
        != lifecycle_ids
        or any(
            row["primary_lifecycle_id"] not in row["lifecycle_ids"]
            or row["expected_outcome"]["intent_id"] is None
            or row["configuration_parameter_ids"]
            or set(row["observation_values"]) != set(row["observation_ids"])
            for row in observations
        )
    ):
        _fail(
            ExecutableAdmissionCode.COVERAGE_MISMATCH,
            pointer="/runtime_bundle/observation_rules",
            detail="decision_or_lifecycle_coverage",
        )
    tick_rows = {
        row["logical_tick"]: row for row in bundle["clock"]["logical_ticks"]
    }
    if len(tick_rows) != 50 or any(
        ...
        _fail(
            ExecutableAdmissionCode.COVERAGE_MISMATCH,
            pointer="/runtime_bundle/clock",
            detail="event_anchor_partial_order_mismatch",
        )
    return counts


def load_singhealth_executable_package(
    path: str | Path,
    *,
    project_root: str | Path | None = None,
    expected_source_sha256: str | None = None,
) -> ExecutableAdmission:
    """Admit an exact package and its referenced runtime bundle."""

    supplied_path = Path(path)
    root = _project_root(supplied_path, project_root)
    package_path = _inside(
        root,
    ...
    if (
        ...
        and package_source_sha != expected_source_sha256
    ):
        _fail(
    ...
    first = build_singhealth_runtime_bundle_document(project_root=root)
    second = build_singhealth_runtime_bundle_document(project_root=root)
    ...
    expected_package = build_singhealth_executable_package_document(
    ...
    if (
        ...
        or package["masim_usage"]["mode"]
        != "read_only_public_interfaces"
