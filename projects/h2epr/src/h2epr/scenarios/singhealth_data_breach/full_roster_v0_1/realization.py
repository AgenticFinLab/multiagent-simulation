"""Deterministic construction of the SingHealth Policy Realization."""
...
from .admission import expected_singhealth_semantic_parent
...
def build_singhealth_policy_realization_document(
    *,
    ...
    if status not in {"candidate", "accepted_policy_realization"}:
        raise ValueError("singhealth_policy_realization_status_invalid")
    ...
    return {
        ...
        "realization_id": "h2epr.0616.policy-realization.v0_1",
        "version": "0.1.0",
        "status": status,
        "event_id": "H2EPR-0616",
...
def _project_root(supplied: str | Path | None) -> Path:
    ...
    if not root.is_dir() or not root.joinpath("src/h2epr").is_dir():
        raise ValueError("singhealth_policy_realization_project_root_invalid")
    return root
...
def _state_update_triggers(
    ...
    if not observed:
        raise ValueError(f"singhealth_state_update_trigger_missing:{state_id}")
    return tuple(sorted(observed))


__all__ = ["build_singhealth_policy_realization_document"]
