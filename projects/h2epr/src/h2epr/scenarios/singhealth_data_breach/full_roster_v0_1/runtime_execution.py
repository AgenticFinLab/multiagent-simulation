"""Deterministic phased execution for the accepted SingHealth runtime bundle."""
...
class SingHealthExecutionError(RuntimeError):
...
def _apply_delta(state: dict[str, Any], payload: Mapping[str, Any]) -> None:
    ...
    if canonical_sha256(target[field_name]) != canonical_sha256(payload["before"]):
        raise SingHealthExecutionError(
            "singhealth_replay_delta_before_mismatch:"
...
def _carry_forward_record(
    *,
    ...
    if (
        ...
        raise SingHealthExecutionError(
            "singhealth_execution_carry_forward_invalid"
        )
    return {
        "object_id": object_id,
        ...
        "next_event_id": "event.0616.post_horizon",
        "terminal": False,
    }


def build_singhealth_run_manifest(admission: ExecutableAdmission) -> dict[str, Any]:
    ...
    preimage = {
        ...
        "run_id": "run.h2epr.0616.canonical.v0_1",
        "event_id": "H2EPR-0616",
...
class SingHealthFullRosterEngine:
    ...
    def __init__(self, admission: ExecutableAdmission) -> None:
        if not admission.accepted or not admission.execution_eligible:
            raise SingHealthExecutionError("singhealth_execution_package_not_eligible")
        self.admission = admission
        self.bundle = _plain(admission.runtime_bundle_document)
        self.manifest = build_singhealth_run_manifest(admission)
        ...
        self.projector = SingHealthObservationProjector(self.bundle)
        self.executor = SingHealthParticipantExecutor()
        self.environment = SingHealthEnvironment(self.bundle)
        self.reducer = AuthoritativeReducer(
            self.bundle["initial_state"], SingHealthReducer(self.bundle).apply_batch
        )
        ...
        self.compiler = SingHealthTraceCompiler()
    ...
    async def setup(self, handles: Mapping[str, Any]) -> None:
        if tuple(sorted(handles)) != self.actor_ids:
            raise SingHealthExecutionError("singhealth_execution_actor_launch_mismatch")

    async def run_tick(
        self, logical_tick: int, barriers: tuple[str, ...]
    ) -> dict[str, Any]:
        if barriers != NAMED_BARRIERS:
            raise SingHealthExecutionError("singhealth_execution_barrier_order_mismatch")
        if logical_tick not in self._clock_by_tick or logical_tick in self.tick_results:
            raise SingHealthExecutionError("singhealth_execution_tick_invalid_or_repeated")
        ...
        if len(deliveries) != consumed_count:
            raise SingHealthExecutionError(
                "singhealth_execution_delivery_consumption_mismatch"
            )
        ...
        for rule in due_rules:
            ...
            if actual != rule["expected_outcome"]:
                raise SingHealthExecutionError(
                    "singhealth_execution_projected_outcome_mismatch:"
    ...
    def _append_completion_records(
        ...
        if unresolved_ids or unresolved_recipients:
            raise SingHealthExecutionError("singhealth_execution_due_message_unresolved")
        self.trace.append(
            "completion",
            logical_tick,
            {
    ...
    def phase_dispatch(self, all_info_lists: list[list[dict[str, Any]]]) -> None:
        if all_info_lists:
            raise SingHealthExecutionError("singhealth_execution_legacy_dispatch_forbidden")

    def get_tick_result(self, logical_tick: int) -> dict[str, Any] | None:
        return copy.deepcopy(self.tick_results.get(logical_tick))

    def finalize(self) -> SingHealthRunArtifacts:
        if self._finalized:
            raise SingHealthExecutionError("singhealth_execution_already_finalized")
        if tuple(sorted(self.tick_results)) != self.logical_ticks:
            raise SingHealthExecutionError("singhealth_execution_tick_coverage_incomplete")
        ...
        if (
            ...
            raise SingHealthExecutionError("singhealth_execution_semantic_coverage_incomplete")
        final_state = self.reducer.state
        unresolved_ids, unresolved_recipients = self.transport.unresolved()
        if unresolved_ids or unresolved_recipients:
            raise SingHealthExecutionError("singhealth_execution_unresolved_transport")
        ...
        if trace_errors:
            raise SingHealthExecutionError(
                "singhealth_execution_trace_invalid:" + ",".join(trace_errors)
            )
        replayed = replay_trace(
            self.bundle["initial_state"], self.trace.records, _apply_delta
        )
        if canonical_sha256(replayed) != canonical_sha256(final_state):
            raise SingHealthExecutionError("singhealth_execution_replay_state_mismatch")
...
class SingHealthFullRosterSimulator(PhasedSimulator):
    """Project binding for the public MASim phased simulator."""


class SingHealthFullRosterRunner(PhasedSimulationRunner):
    simulator_class = SingHealthFullRosterSimulator
...
async def _materialize(
    ...
    if operational_root.exists() and any(operational_root.iterdir()):
        raise FileExistsError("singhealth_operational_root_must_be_fresh")
    operational_root.mkdir(parents=True, exist_ok=True)
    engine = SingHealthFullRosterEngine(admission)
    config = SimulationConfig(
        setting={
            "name": "h2epr_0616_full_roster_rule",
    ...
    runner = SingHealthFullRosterRunner(config)
    results = await runner.execute()
    if len(results) != len(engine.logical_ticks):
        raise SingHealthExecutionError("singhealth_execution_result_tick_count_mismatch")
    return engine.finalize()


def materialize_singhealth_run(
    admission: ExecutableAdmission,
...
__all__ = [
    "SingHealthExecutionError",
    "SingHealthFullRosterEngine",
    "SingHealthFullRosterRunner",
    "SingHealthFullRosterSimulator",
    "SingHealthRunArtifacts",
    "build_singhealth_run_manifest",
