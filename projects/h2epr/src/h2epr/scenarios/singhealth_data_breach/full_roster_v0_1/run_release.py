"""Compact custody and release boundary for the SingHealth full-roster run."""
...
from .runtime_execution import materialize_singhealth_run


RUN_RELEASE_ID = "H2EPR-0616-RUN-AND-GRAPH-v0.1"
...
COMPARISON_IDENTITY = RunComparisonIdentity(
    event_id="H2EPR-0616",
    comparison_id="h2epr.0616.run-comparison.canonical.v0_1",
)
CUSTODY_IDENTITY = RunCustodyIdentity(
    event_id="H2EPR-0616",
    guide_title="SingHealth Data Breach run custody v0.1",
)
...
IMPLEMENTATION_SOURCE_PATHS = (
    ...
    Path(
        "src/h2epr/scenarios/singhealth_data_breach/"
...
class SingHealthRunReleaseCode(str, Enum):
    """Stable fail-closed classes for compact run-release admission."""

    SOURCE_NOT_FOUND = "SINGHEALTH_RUN_RELEASE_SOURCE_NOT_FOUND"
    PATH_UNSAFE = "SINGHEALTH_RUN_RELEASE_PATH_UNSAFE"
    JSON_INVALID = "SINGHEALTH_RUN_RELEASE_JSON_INVALID"
    JSON_DUPLICATE_KEY = "SINGHEALTH_RUN_RELEASE_JSON_DUPLICATE_KEY"
    INTEGRITY_MISMATCH = "SINGHEALTH_RUN_RELEASE_INTEGRITY_MISMATCH"
    IDENTITY_MISMATCH = "SINGHEALTH_RUN_RELEASE_IDENTITY_MISMATCH"
    ARTIFACT_SET_MISMATCH = "SINGHEALTH_RUN_RELEASE_ARTIFACT_SET_MISMATCH"
    EXECUTABLE_PARENT_MISMATCH = (
        "SINGHEALTH_RUN_RELEASE_EXECUTABLE_PARENT_MISMATCH"
    )
    CLOSURE_MISMATCH = "SINGHEALTH_RUN_RELEASE_CLOSURE_MISMATCH"


class SingHealthRunReleaseError(ValueError):
    """One typed compact run-release rejection."""

    def __init__(
        self,
        code: SingHealthRunReleaseCode,
        *,
        pointer: str = "",
        detail: str = "",
    ) -> None:
...
class SingHealthRunReleaseAdmission:
...
def _fail(
    ...
    *,
    ...
    raise SingHealthRunReleaseError(code, pointer=pointer, detail=detail)
...
def _project_root(path: Path, supplied: str | Path | None) -> Path:
    ...
    if not root.is_dir() or not root.joinpath("src/h2epr").is_dir():
        _fail(SingHealthRunReleaseCode.PATH_UNSAFE, detail="project_root")
    return root
...
def _map_io_error(error: ExecutionIOError) -> None:
    codes = {
        ExecutionIOCode.SOURCE_NOT_FOUND: (
            SingHealthRunReleaseCode.SOURCE_NOT_FOUND
        ),
        ExecutionIOCode.PATH_UNSAFE: SingHealthRunReleaseCode.PATH_UNSAFE,
        ExecutionIOCode.JSON_INVALID: SingHealthRunReleaseCode.JSON_INVALID,
...
def materialize_singhealth_run_pair(
    admission: ExecutableAdmission,
    custody_root: str | Path,
) -> RunPair:
    """Produce and preserve two fresh SingHealth materializations."""

    return materialize_run_pair(
        lambda operational_root: materialize_singhealth_run(
...
def build_singhealth_formal_run_documents(
    run_pair: RunPair,
) -> dict[str, Any]:
...
def build_singhealth_run_release_manifest(
    formal_documents: Mapping[str, Any],
    *,
    ...
    if set(formal_documents) != set(FORMAL_RUN_DOCUMENTS):
        _fail(SingHealthRunReleaseCode.ARTIFACT_SET_MISMATCH)
    comparison = formal_documents["determinism-comparison.json"]
    if comparison.get("status") != "pass":
        _fail(SingHealthRunReleaseCode.CLOSURE_MISMATCH)
    ...
    if any(not (release_root / path).is_file() for _, path in publication_paths):
        _fail(SingHealthRunReleaseCode.SOURCE_NOT_FOUND, pointer="/publication")
    ...
    return {
        ...
        "next_stage": {
            "name": "cross_event_execution_and_publication_closeout",
            "singhealth_run_closed": True,
            "shared_kernel_second_consumer": True,
        },
    }
...
def load_singhealth_run_release(
    path: str | Path,
    *,
    project_root: str | Path | None = None,
    expected_manifest_source_sha256: str | None = None,
) -> SingHealthRunReleaseAdmission:
    ...
    if (
        ...
        _fail(
            SingHealthRunReleaseCode.INTEGRITY_MISMATCH,
            pointer="/manifest",
    ...
    if (
        ...
        or manifest.get("event_id") != "H2EPR-0616"
        or manifest.get("run_id") != "run.h2epr.0616.canonical.v0_1"
        ...
        _fail(SingHealthRunReleaseCode.IDENTITY_MISMATCH, pointer="/manifest")
    ...
    if (
        ...
        _fail(
            SingHealthRunReleaseCode.ARTIFACT_SET_MISMATCH,
    ...
    return SingHealthRunReleaseAdmission(
        release_id=RUN_RELEASE_ID,
        version=RUN_RELEASE_VERSION,
        event_id="H2EPR-0616",
