"""Event-neutral deterministic participant Rule interface."""
...
class ParticipantDecisionContext:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "private_state",
            MappingProxyType(dict(self.private_state)),
        )
        object.__setattr__(
            self,
...
class RuleParticipantPolicy:
    ...
    def _validate(self) -> None:
        ...
        for decision in self._decisions.values():
            ...
            for branch in decision.branches:
                ...
                updates = tuple(
                    field_id for field_id, _ in branch.private_state_updates
                )
                if (
                    not branch.when_all
            ...
            if any(
                _matches(branch, decision.baseline_facts)
                for branch in decision.branches
            ):
                raise ParticipantPolicyError(
    ...
    def witness_context(
        ...
        *,
        ...
        return ParticipantDecisionContext(
            ...
            observations={key: facts[key] for key in decision.observation_ids},
            private_state={key: facts[key] for key in decision.private_state_ids},
            configuration_parameters=configured,
        )
    ...
    def decide(self, context: ParticipantDecisionContext) -> ParticipantDecision:
        ...
        return ParticipantDecision(
            ...
            lifecycle_ids=decision.lifecycle_ids,
            proposed_private_state_updates=dict(branch.private_state_updates),
        )
