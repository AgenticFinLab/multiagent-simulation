# Mainland China Note7 Test-Unit Owners Population Model

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0481.population.china_test_unit_owners.v1` |
| Choice unit | one aggregate request interface for the mainland test-unit cohort identified as P_5 |
| Runtime representation | one aggregate Population actor; no individual agents or weighted samples |
| Actor ID | `china_test_unit_owners` |
| Benchmark | H2EPR-0481, August 2016–January 2017 represented boundary |
| Representation | population; bounded test-unit return/refund-request interface |
| Source ID | `P_5` |
| Primary choices | Submit one aggregate modeled return/refund request after receiving the limited-recall notice. |
...
The Draft exposes eligibility and an explicit refund-or-replacement option rather than observed uptake. The Population therefore records a constructed request choice and labels it as such; it does not claim that all 1,858 owners acted.

No completed return, payment, replacement delivery, participation rate or individual consumer trajectory is represented. The parent fixes no calibrated utility, personality,
...
The represented test-unit cohort excludes regular mainland purchasers and the separately represented global cohort. The number 1,858 describes recall scope, not a sampled population size or a participation weight.
...
Frozen anchors: SRC004 and SRC007. The Draft compensation transaction points to the regulator rather than P_5. Actor-local cohort identity and transaction prose support only the qualified request boundary.
...
This population may submit one aggregate modeled return/refund request after receiving the limited-recall notice. It cannot act as another producer,
...
An accepted request does not settle a refund. P_5 cannot issue the recall notice or update manufacturer compliance.
...
Without actual delivery of Samsung's limited-recall notice, the selected request row waits; eligibility is not treated as received information. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `request_test_unit_return` | known `test_unit_recall_notice` from `samsung_electronics` | Record one aggregate modeled request; no refund, return or replacement is completed. |
...
| --- | --- | --- |
| `request_test_unit_return` | `consumer_requests` | `consumer_requests.test_unit_return`: unrecorded → `aggregate_request_recorded` |
...
- Normal: After notice delivery, the Population records one aggregate request and sends it to Samsung for information only.
- Missing information: Without actual delivery of Samsung's limited-recall notice, the selected request row waits; eligibility is not treated as received information.
...
- Authority/adverse case: An accepted request does not settle a refund. P_5 cannot issue the recall notice or update manufacturer compliance.
- Perturbation: Suppressing the notice leaves request status unrecorded while the limited-recall announcement itself remains valid.

- Aggregate alternatives: Missing notice keeps the aggregate request unrecorded; a received notice permits the selected request without proving any individual returned a device.
- Aggregation change: Splitting return versus replacement preferences would require admitted choice data and distinct response semantics; duplicating this actor would double the request record rather than estimate uptake.
...
No completed return, payment, replacement delivery, participation rate or individual consumer trajectory is represented. The Draft compensation transaction points to the regulator rather than P_5. Actor-local cohort identity and transaction prose support only the qualified request boundary.
