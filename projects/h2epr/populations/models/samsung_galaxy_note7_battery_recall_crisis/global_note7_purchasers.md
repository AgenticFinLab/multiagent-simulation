# Global Note7 Purchasers outside Mainland China Population Model

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0481.population.global_note7_purchasers.v1` |
| Actor ID | `global_note7_purchasers` |
| Benchmark | H2EPR-0481, August 2016–January 2017 represented boundary |
| Representation | population; aggregate early incident-report interface |
| Source ID | `P_3` |
| Primary choices | Record the represented early incident reports and communicate them to Samsung. |
...
P_3's two names describe the same global purchaser cohort before and after recall eligibility. The Population preserves aggregate reporting without inventing individual devices or unanimous behavior.

Recall eligibility is not population-wide return, refund, exchange, injury or verified incident prevalence. The parent fixes no calibrated utility, personality,
...
Frozen anchors: SRC001, SRC003, SRC004, SRC005 and SRC009. Frozen pages report inconsistent incident counts and severity; the model records that reports occurred, not their verified number or physical cause.
...
This population may record the represented early incident reports and communicate them to Samsung. It cannot act as another producer,
...
The cohort cannot issue a Samsung statement or change a recall field. Its report may be received without forcing a particular manufacturer response.
...
If the report is absent or delayed, guarded Samsung response rows wait; the environment does not infer a report from the Draft stage label. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `report_early_battery_incidents` | `product.sales_start` = `recorded` | Record aggregate early incident reporting, without verifying count, severity or cause. |
...
| --- | --- | --- |
| `report_early_battery_incidents` | `early_incidents` | `early_incidents.consumer_report`: unrecorded → `recorded` |
...
- Normal: The aggregate report is accepted once, sent to Samsung and retained in Samsung's received-memory after delivery.
- Missing information: If the report is absent or delayed, guarded Samsung response rows wait; the environment does not infer a report from the Draft stage label.
...
Recall eligibility is not population-wide return, refund, exchange, injury or verified incident prevalence. Frozen pages report inconsistent incident counts and severity; the model records that reports occurred, not their verified number or physical cause.
