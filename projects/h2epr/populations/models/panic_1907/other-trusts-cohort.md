# Other New York trust companies — Population Model

## 1. Model overview

| Field | Account |
|---|---|
| Semantic ID | `h2epr.0288.population.other_trusts_cohort.v0.4` |
| Runtime actor | `other_trusts_cohort` |
| Benchmark event | Panic of 1907 (`H2EPR-0288`) |
| Source participant | `P_10` — New York Other Trust Companies Group (includes Trust Company of America) |
| Representation | `population` |
| Event role | secondary run targets |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Population scope and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. The cohort suppresses institution-level differences.
...
- `draft_epg:S2/E5/P_10`
- `draft_epg:S2/E6/P_10`
- `draft_epg:S3/E8/P_10`
...
Role: secondary run targets.

Authority boundary: Carries group-level run and call-balance liquidation transitions.
