# Lebanese National Movement Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0892.agent.lebanese_national_movement.v1` |
| Actor ID | `lebanese_national_movement` |
| Benchmark | H2EPR-0892, 1975-04-13 through the Draft's qualified 1990-10-13 post-war boundary |
| Representation | agent; three early LNM coalition participation records |
| Source ID | `P_2` |
| Primary choices | Record initial armed participation, Beirut territorial campaigning and continued conflict during the early Syrian-intervention phase. |
...
P_2 appears in the first three episodes as an organizational coalition. Its current interface ends with the exposed 1976–1978 record and does not project the label into later factions or infer internal members.

It cannot decide for the PLO, Lebanese Front, Syria, civilians or later militias, produce territorial control, quantify violence, or establish one coherent Muslim or leftist preference. The parent fixes no calibrated utility, personality,
...
Frozen anchors: SRC002, SRC004, SRC005, SRC007 and SRC008. Several sources reduce the war to a binary Christian/Muslim conflict. The model retains P_2 and P_3 separately; malformed aid transactions do not turn Egypt, Libya or the government into P_2's runtime owner.
...
This agent may record initial armed participation, Beirut territorial campaigning and continued conflict during the early Syrian-intervention phase. It cannot act as another producer,
...
An accepted participation record does not produce casualties, territory or victory and does not compel Syria to change alignment.
...
Territorial and continued records wait for delivered early-counterparty and Syrian-deployment information respectively. Closed windows remain legitimate open endpoints. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_initial_lnm_participation` | source-bounded availability and own record not yet made | Record the LNM's qualified initial conflict participation without treating a coalition as all Muslim or leftist actors. |
| `record_lnm_beirut_campaign` | known `front_initial_record` from `lebanese_front_record_interface` | Record the represented LNM territorial campaign without producing territory or civilian harm. |
| `record_continued_lnm_conflict` | known `syrian_deployment_notice` from `syrian_state_intervention_interface` | Record continued LNM conflict after the Syrian deployment notice, not military strength or victory. |
...
| --- | --- | --- |
| `record_initial_lnm_participation` | `initial_conflict` | `initial_conflict.lnm_participation`: unrecorded → `qualified_lnm_participation_recorded` |
| `record_lnm_beirut_campaign` | `beirut_territorial_records` | `beirut_territorial_records.lnm_campaign`: unrecorded → `beirut_campaign_recorded` |
| `record_continued_lnm_conflict` | `continued_conflict` | `continued_conflict.lnm_record`: unrecorded → `continued_lnm_conflict_recorded` |
...
- Normal: The LNM records its initial and Beirut participation, then a continued-conflict record after the Syrian deployment notice is delivered.
- Missing information: Territorial and continued records wait for delivered early-counterparty and Syrian-deployment information respectively. Closed windows remain legitimate open endpoints.
...
It cannot decide for the PLO, Lebanese Front, Syria, civilians or later militias, produce territorial control, quantify violence, or establish one coherent Muslim or leftist preference. Several sources reduce the war to a binary Christian/Muslim conflict. The model retains P_2 and P_3 separately; malformed aid transactions do not turn Egypt, Libya or the government into P_2's runtime owner.
