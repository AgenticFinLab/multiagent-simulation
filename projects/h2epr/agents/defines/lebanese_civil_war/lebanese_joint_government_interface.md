# Lebanese Joint-Government Record Interface Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0892.agent.lebanese_joint_government_interface.v1` |
| Actor ID | `lebanese_joint_government_interface` |
| Benchmark | H2EPR-0892, 1975-04-13 through the Draft's qualified 1990-10-13 post-war boundary |
| Representation | agent; one qualified intervention-request record |
| Source ID | `P_6` |
| Primary choices | Record the represented request for Syrian intervention after receiving the three territorial-conflict records. |
...
P_6 appears twice as a weakened joint government. The Draft combines failure, collapse and a request while its narrative attributes the request to Christian leaders; the representation gate retains only a qualified public request record.

It cannot choose state collapse, represent unanimous government consent, command Syria, restore order, act for the Lebanese Front or create a military outcome. The parent fixes no calibrated utility, personality,
...
| --- | --- | --- |
| draft_epg:S1/E1/P_6 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |
| draft_epg:S1/E2/P_6 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC005 and SRC007, with broad SRC002/SRC004 context. The Draft conflicts on whether and when the government formally collapsed and who requested intervention. Current semantics expose the ambiguity rather than upgrading it to a precise constitutional decision.
...
This agent may record the represented request for Syrian intervention after receiving the three territorial-conflict records. It cannot act as another producer,
...
A request record neither causes deployment nor validates the requester, mandate, motive or legality of intervention.
...
The gate waits for delivered territorial records from P_1, P_2 and P_3. Missing one record leaves the request unrecorded. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_syrian_intervention_request` | known `front_beirut_campaign_record` from `lebanese_front_record_interface`; known `lnm_beirut_campaign_record` from `lebanese_national_movement`; known `plo_beirut_position_record` from `palestine_liberation_organization` | Record the ambiguous intervention request without choosing government collapse or compelling deployment. |
...
| --- | --- | --- |
| `record_syrian_intervention_request` | `state_authority` | `state_authority.intervention_request`: unrecorded → `qualified_syrian_intervention_request_recorded` |
...
- Normal: After the three competing territorial records arrive, the gate publishes one qualified intervention request to the Syrian interface.
- Missing information: The gate waits for delivered territorial records from P_1, P_2 and P_3. Missing one record leaves the request unrecorded.
...
- Authority/adverse case: A request record neither causes deployment nor validates the requester, mandate, motive or legality of intervention.
- Perturbation: Suppressing one territorial notice prevents this request and its information-dependent foreign-intervention path without rewriting initial conflict records.
...
It cannot choose state collapse, represent unanimous government consent, command Syria, restore order, act for the Lebanese Front or create a military outcome. The Draft conflicts on whether and when the government formally collapsed and who requested intervention. Current semantics expose the ambiguity rather than upgrading it to a precise constitutional decision.
