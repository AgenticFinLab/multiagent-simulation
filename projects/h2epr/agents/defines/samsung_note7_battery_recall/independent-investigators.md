# Independent third-party investigation consortium — Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic ID | `h2epr.0481.agent.independent_investigators.v0.4` |
| Runtime actor | `independent_investigators` |
| Benchmark event | Samsung Galaxy Note7 Battery Recall (`H2EPR-0481`) |
| Source participant | `P_9` — Independent Third-Party Investigation Consortium (UL, Exponent, TÜV Rheinland) |
| Representation | `agent` |
| Event role | independent investigation body |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. Experimental methods and evidentiary uncertainty are not reconstructed.
...
Role: independent investigation body.

Authority boundary: May produce the exposed supplier-battery findings after commission.
...
- `investigate_supplier_batteries`
- `no_op`
