...
| Field | Account |
|---|---|
| Semantic ID | `h2epr.0481.agent.samsung.v0.4` |
| Runtime actor | `samsung` |
| Benchmark event | Samsung Galaxy Note7 Battery Recall (`H2EPR-0481`) |
| Source participant | `P_1` — Samsung Electronics Co., Ltd. (三星电子) |
| Representation | `agent` |
| Event role | manufacturer and program owner |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. Internal governance, engineering evidence, and counterfactual choices are not reconstructed.
...
Role: manufacturer and program owner.

Authority boundary: May execute the launch, recall, production, investigation, and publication transitions exposed by the Draft EPG.
...
- `entities.cause_report.status`
- `entities.corporate_position.china_claim`
- `entities.corporate_response.shipment_status`
- `entities.corporate_response.testing_status`
- `entities.investigation.status`
- `entities.product.launch_status`
- `entities.production.status`
- `entities.recall.china_scope`
- `entities.recall.global_scope`
- `entities.sales.global_status`

## 6. Admissible decision semantics

The backend may choose only:

- `announce_partial_global_recall`
- `apologize_china_consumers`
- `begin_internal_investigation`
- `initiate_testing_and_delay`
- `issue_external_heating_claim`
- `launch_product`
- `no_op`
- `publish_cause_report`
- `recall_china_test_units`
- `start_global_sales`
- `suspend_production`
- `terminate_production_and_sales`
