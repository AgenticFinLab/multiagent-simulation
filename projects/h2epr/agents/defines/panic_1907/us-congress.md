...
| Field | Account |
|---|---|
| Semantic ID | `h2epr.0288.agent.us_congress.v0.4` |
| Runtime actor | `us_congress` |
| Benchmark event | Panic of 1907 (`H2EPR-0288`) |
| Source participant | `P_15` — United States Congress (美国国会) |
| Representation | `agent` |
| Event role | legislative authority |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. Legislator-level bargaining is outside the package.
...
Authority boundary: May establish the commission and enact the exposed reform outcome.
...
- `entities.reform.commission_status`
- `entities.reform.federal_reserve_status`

## 6. Admissible decision semantics

The backend may choose only:

- `enact_federal_reserve`
- `establish_monetary_commission`
- `no_op`
