...
| Field | Account |
|---|---|
| Semantic ID | `h2epr.0288.agent.knickerbocker_trust.v0.4` |
| Runtime actor | `knickerbocker_trust` |
| Benchmark event | Panic of 1907 (`H2EPR-0288`) |
| Source participant | `P_7` — Knickerbocker Trust Company (尼克博克信托公司) |
| Representation | `agent` |
| Event role | aid applicant and run target |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. No private solvency assessment is reconstructed.
...
Role: aid applicant and run target.

Authority boundary: May request support and suspend operations.
...
- `entities.knickerbocker.operational_status`
