# J.P. Morgan — Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic ID | `h2epr.0288.agent.jp_morgan.v0.4` |
| Runtime actor | `jp_morgan` |
| Benchmark event | Panic of 1907 (`H2EPR-0288`) |
| Source participant | `P_9` — J.P. Morgan (约翰·皮尔庞特·摩根) |
| Representation | `agent` |
| Event role | private-sector coordinator |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. Private deliberation and counterfactual financing capacity are absent.
...
Role: private-sector coordinator.

Authority boundary: May answer support requests and coordinate the declared rescue actions.
...
- `entities.call_market.liquidity_status`
- `entities.knickerbocker.support_status`
- `entities.nyse.liquidity_support`
- `entities.private_rescue.status`
...
- `begin_private_rescue`
- `deny_emergency_support`
- `lead_trust_bailout`
- `no_op`
- `support_exchange_liquidity`
