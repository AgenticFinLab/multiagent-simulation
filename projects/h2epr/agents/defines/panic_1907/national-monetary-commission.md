...
| Field | Account |
|---|---|
| Semantic ID | `h2epr.0288.agent.national_monetary_commission.v0.4` |
| Runtime actor | `national_monetary_commission` |
| Benchmark event | Panic of 1907 (`H2EPR-0288`) |
| Source participant | `P_16` — National Monetary Commission (国家货币委员会) |
| Representation | `agent` |
| Event role | reform advisory body |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. The report's internal drafting process is not reconstructed.
...
Role: reform advisory body.

Authority boundary: May issue the exposed recommendation report.
...
- `entities.reform.recommendation_status`

## 6. Admissible decision semantics

The backend may choose only:

- `issue_reform_recommendations`
- `no_op`
