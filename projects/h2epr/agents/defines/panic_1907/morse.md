# Charles W. Morse — Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic ID | `h2epr.0288.agent.morse.v0.4` |
| Runtime actor | `morse` |
| Benchmark event | Panic of 1907 (`H2EPR-0288`) |
| Source participant | `P_2` — Charles W. Morse (查尔斯·w·莫尔斯/Morse) |
| Representation | `agent` |
| Event role | market-corner initiator |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. The dataset does not separate his private decision process from the exposed event outline.
...
Role: market-corner initiator.

Authority boundary: May initiate the declared United Copper action.
