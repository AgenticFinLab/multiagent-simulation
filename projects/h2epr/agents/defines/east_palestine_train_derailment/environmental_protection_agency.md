# U.S. Environmental Protection Agency decision interface

## 1. Model overview

| Field | Account |
|---|---|
| Agent ID and display name | `environmental_protection_agency` — U.S. Environmental Protection Agency decision interface |
| Benchmark event and interval | `H2EPR-0196`; 2023-02-03 through the Draft's 2025-01-28 endpoint |
| Represented decision interface | the federal environmental oversight choice to initiate the exposed cleanup regime |
| Source participant IDs | `P_6` |
| Primary decision situations | starting cleanup oversight after response and health signals are available |
...
This Agent represents the federal environmental oversight choice to initiate the exposed cleanup regime. It treats the named organization or coordinated command as one public decision interface and does not synthesize internal staff, private deliberation, or undisclosed authority. It excludes railroad execution, state health policy, settlement decisions, and a finding of cleanup effectiveness. A successor must split or narrow the Agent when the dataset supports independently acting internal units whose choices change the process.
...
EPA reads the response instruction and resident concern report, then may open the represented cleanup oversight and notify the railroad. The two messages can arrive at different ticks. EPA has oversight authority in this abstraction; the railroad owns its response to the directive, and neither owns a scientific finding about cleanup effectiveness.
...
`start_cleanup_oversight` uses retained `response_instruction` and `health_report` information plus the corresponding public record preconditions. Requiring both messages is an explicit construction choice, not an assertion that resident reporting is a legal prerequisite for EPA action. Waiting for the second message preserves the first. The Rule row is available from the cleanup interval and completes once accepted; no successful physical remediation is inferred.
...
- Deliver the instruction at c04 and health report at c06: both are known when oversight is considered at c06.
- Withhold the health report: oversight can remain open under this chosen baseline, exposing the sensitivity of the authored dependency.
- A rejected oversight request is known next tick and is reconsidered only after observed information changes.
- A cleanup directive does not imply railroad acceptance, completed removal, or verified environmental safety.
...
The dataset does not expose internal decision records, calibrated behavior, or counterfactual choices. Railroad execution, state health policy, settlement decisions, and a finding of cleanup effectiveness. A successor is required if new admissible dataset content changes the represented authority, actor cardinality, or information boundary. The anchors in Section 3 are the complete Draft basis for this parent.
