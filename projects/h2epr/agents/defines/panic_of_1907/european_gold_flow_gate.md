# European Gold-Flow Representation Gate Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0288.agent.european_gold_flow_gate.v1` |
| Actor ID | `european_gold_flow_gate` |
...
| Representation | agent; qualified aggregate record of European gold exports/inflows following the represented currency premium |
| Source ID | `P_13` |
| Primary choices | Record the dataset's aggregate gold-flow account after receiving the public convertibility-suspension signal. |
...
P_13 names London, Paris and Berlin as a group rather than a decision-making organization. A representation gate prevents the aggregate narrative from becoming a calibrated market or uniform institutional policy.

The gate cannot choose for named banks, conserve gold, set exchange rates, transmit the crisis, prove that imports restored liquidity, or expose an unobserved trade mechanism. The parent fixes no calibrated utility, personality,
...
| --- | --- | --- |
| draft_epg:S3/E9/P_13 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC003 and the repeated SRC007/SRC009/SRC011 account. The Draft states both export and systemic restoration as a participant action. Current state records a qualified aggregate flow account only; restoration remains a descriptive claim.
...
This agent may record the dataset's aggregate gold-flow account after receiving the public convertibility-suspension signal. It cannot act as another producer,
...
The record is not a causal estimate and does not set US financial-system health or end the panic.
...
The gate waits for the delivered public suspension signal. If absent, the gold-flow coordinate remains legitimately unrecorded. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_aggregate_gold_inflow_account` | known `currency_premium_signal` from `ny_clearing_house_member_bank_population` | Record the dataset's qualified aggregate gold-inflow account after the public suspension signal; do not write liquidity restoration. |
...
| --- | --- | --- |
| `record_aggregate_gold_inflow_account` | `gold_flow` | `gold_flow.european_imports`: unrecorded → `qualified_inflow_recorded` |
...
- Normal: After member suspension becomes public, the gate records the qualified gold-inflow account without creating recovery state.
- Missing information: The gate waits for the delivered public suspension signal. If absent, the gold-flow coordinate remains legitimately unrecorded.
...
- Authority/adverse case: The record is not a causal estimate and does not set US financial-system health or end the panic.
- Perturbation: Missing convertibility coordination leaves gold flow and the chosen reform activation path open, exposing the authored dependency.
...
The gate cannot choose for named banks, conserve gold, set exchange rates, transmit the crisis, prove that imports restored liquidity, or expose an unobserved trade mechanism. The Draft states both export and systemic restoration as a participant action. Current state records a qualified aggregate flow account only; restoration remains a descriptive claim.
