# F. Augustus Heinze Scheme Interface Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0288.agent.augustus_heinze_scheme_interface.v1` |
| Actor ID | `augustus_heinze_scheme_interface` |
...
| Representation | agent; Heinze's represented participation in the failed United Copper stock-corner attempt |
| Source ID | `P_1` |
| Primary choices | Record Heinze's bounded participation in the joint stock scheme and disclose the qualified failure record. |
...
P_1 persists across E1–E3, but only the actor-local scheme participation is retained as a selectable record. Later bank runs and forced resignation are consequences or third-party actions.

This interface cannot set the stock outcome, cause withdrawals, control affiliated banks, resign another manager, or determine the crisis trajectory. The parent fixes no calibrated utility, personality,
...
| draft_epg:S1/E3/P_1 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC002, SRC007, SRC009 and SRC011. The Draft labels P_1 an initiator/perpetrator and later treats indirect causation and forced resignation as actions. The current parent retains participation without converting narrative causation or compulsion into authority.
...
This agent may record Heinze's bounded participation in the joint stock scheme and disclose the qualified failure record. It cannot act as another producer,
...
A recorded participation does not itself create a run, insolvency, loss amount or resignation. P_2 remains a separate decision owner.
...
The participation row has no incoming runtime prerequisite. If omitted, depositor rows requiring both disclosed scheme records wait rather than receiving an environment default. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_heinze_scheme_participation` | source-bounded availability and own record not yet made | Record Heinze's represented participation in the failed joint United Copper scheme, not the market mechanism or downstream crisis. |
...
| --- | --- | --- |
| `record_heinze_scheme_participation` | `scheme` | `scheme.heinze_participation`: unrecorded → `qualified_failed_scheme_recorded` |
...
- Normal: The interface records one qualified participation and sends the public failure record to the depositor and Knickerbocker interfaces.
- Missing information: The participation row has no incoming runtime prerequisite. If omitted, depositor rows requiring both disclosed scheme records wait rather than receiving an environment default.
...
- Authority/adverse case: A recorded participation does not itself create a run, insolvency, loss amount or resignation. P_2 remains a separate decision owner.
- Perturbation: Withholding this row leaves both joint-record-dependent withdrawal paths open without invalidating unrelated post-crisis rows.
...
This interface cannot set the stock outcome, cause withdrawals, control affiliated banks, resign another manager, or determine the crisis trajectory. The Draft labels P_1 an initiator/perpetrator and later treats indirect causation and forced resignation as actions. The current parent retains participation without converting narrative causation or compulsion into authority.
