# Charles W. Morse Scheme Interface Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0288.agent.charles_morse_scheme_interface.v1` |
| Actor ID | `charles_morse_scheme_interface` |
...
| Representation | agent; Morse's represented participation in the failed United Copper stock-corner attempt |
| Source ID | `P_2` |
| Primary choices | Record Morse's bounded participation in the joint stock scheme and disclose the qualified failure record. |
...
P_2 is kept separate from P_1 because the Draft assigns a distinct identity and appearances. Its current choice is one participation record, not a complete market-manipulation mechanism.

This interface cannot set the stock outcome, cause withdrawals, control affiliated banks, resign another manager, or determine later rescue and reform. The parent fixes no calibrated utility, personality,
...
| draft_epg:S1/E3/P_2 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC002, SRC007, SRC009 and SRC011. Draft indirect-trigger and forced-resignation rows are narrative results. They do not authorize P_2 to write depositor, bank or clearing-house state.
...
This agent may record Morse's bounded participation in the joint stock scheme and disclose the qualified failure record. It cannot act as another producer,
...
Morse cannot act for Heinze or make a joint record sufficient when Heinze's separately represented participation is absent.
...
The row is source-bounded and independent of incoming messages. Its absence keeps dependent depositor decisions open. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_morse_scheme_participation` | source-bounded availability and own record not yet made | Record Morse's represented participation in the failed joint United Copper scheme without writing another actor's choice or a causal outcome. |
...
| --- | --- | --- |
| `record_morse_scheme_participation` | `scheme` | `scheme.morse_participation`: unrecorded → `qualified_failed_scheme_recorded` |
...
- Normal: The interface records its own participation and sends the same qualified public failure record through separately attributable transport.
- Missing information: The row is source-bounded and independent of incoming messages. Its absence keeps dependent depositor decisions open.
...
- Authority/adverse case: Morse cannot act for Heinze or make a joint record sufficient when Heinze's separately represented participation is absent.
- Perturbation: Missing Morse participation has the same fail-open behavioral consequence as missing Heinze participation while preserving actor isolation.
...
This interface cannot set the stock outcome, cause withdrawals, control affiliated banks, resign another manager, or determine later rescue and reform. Draft indirect-trigger and forced-resignation rows are narrative results. They do not authorize P_2 to write depositor, bank or clearing-house state.
