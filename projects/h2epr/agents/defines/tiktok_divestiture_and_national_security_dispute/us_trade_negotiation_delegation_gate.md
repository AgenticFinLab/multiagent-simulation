# United States Trade-Negotiation Delegation Gate Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0170.agent.us_trade_negotiation_delegation_gate.v1` |
| Actor ID | `us_trade_negotiation_delegation_gate` |
| Benchmark | H2EPR-0170, 2020-08-06 through the Draft's qualified 2025-09-15 negotiation and grace-extension endpoint |
| Representation | agent; the qualified US-side position in the represented fourth-round talks |
| Source ID | `P_10` |
| Primary choices | Record the US delegation's qualified data-operation and algorithm-authorization consensus position after receiving the corporate proposal and grace notice. |
...
P_10 appears only in E10 as a government group. The gate represents the exposed negotiating position, not membership, internal authorization, presidential approval or the full US government.

The gate cannot bind ByteDance, the Chinese delegation or Trump, waive or enforce law, choose an operator, transfer ownership, or implement platform governance. The parent fixes no calibrated utility, personality,
...
| --- | --- | --- |
| draft_epg:S4/E10/P_10 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC009. The Draft says the US side abandoned full divestment but provides one later commentary account and a relation that omits P_10. Current value is a qualified position, not verified legal waiver.
...
This agent may record the US delegation's qualified data-operation and algorithm-authorization consensus position after receiving the corporate proposal and grace notice. It cannot act as another producer,
...
One position neither establishes bilateral consensus nor changes the enacted-law, service or ownership fields.
...
The gate waits for actual grace and proposal messages. Missing information leaves its own and downstream records open. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_us_consensus_position` | known `negotiation_grace_notice` from `donald_trump_executive_interface`; known `bytedance_resolution_proposal` from `bytedance_platform_governance_interface` | Record only the US delegation's qualified position after grace and corporate proposal are known. |
...
| --- | --- | --- |
| `record_us_consensus_position` | `negotiation` | `negotiation.us_delegation_position`: unrecorded → `qualified_consensus_position_recorded` |
...
- Normal: The gate publishes its separately attributable qualified position to ByteDance, the Chinese delegation and Trump.
- Missing information: The gate waits for actual grace and proposal messages. Missing information leaves its own and downstream records open.
...
- Authority/adverse case: One position neither establishes bilateral consensus nor changes the enacted-law, service or ownership fields.
- Perturbation: Withholding its position prevents ByteDance's two-position resolution row while leaving the Chinese position visible.
...
The gate cannot bind ByteDance, the Chinese delegation or Trump, waive or enforce law, choose an operator, transfer ownership, or implement platform governance. The Draft says the US side abandoned full divestment but provides one later commentary account and a relation that omits P_10. Current value is a qualified position, not verified legal waiver.
