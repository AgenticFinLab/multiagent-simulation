# China Trade-Negotiation Delegation Gate Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0170.agent.china_trade_negotiation_delegation_gate.v1` |
| Actor ID | `china_trade_negotiation_delegation_gate` |
| Benchmark | H2EPR-0170, 2020-08-06 through the Draft's qualified 2025-09-15 negotiation and grace-extension endpoint |
| Representation | agent; the qualified Chinese-side position in the represented fourth-round talks |
| Source ID | `P_9` |
| Primary choices | Record the Chinese delegation's qualified data-operation and algorithm-authorization consensus position after receiving the corporate proposal and grace notice. |
...
P_9 appears only in E10 as a government group. A representation gate preserves the source-level position without inventing members, internal authority, bargaining strategy or a general state actor.

The gate cannot bind ByteDance, the US delegation or either government, transfer algorithm rights, approve an operator, implement data hosting, or establish a final agreement alone. The parent fixes no calibrated utility, personality,
...
| --- | --- | --- |
| draft_epg:S4/E10/P_9 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC009. The final Draft relation names only Trump and ByteDance as consensus counterparties despite separate delegation rows. Current semantics require distinct P_9/P_10 positions before any ByteDance resolution record.
...
This agent may record the Chinese delegation's qualified data-operation and algorithm-authorization consensus position after receiving the corporate proposal and grace notice. It cannot act as another producer,
...
One side's position is not bilateral agreement or implementation and creates no ownership or operational effect.
...
The gate waits for both the grace notice and ByteDance proposal. If either is absent, its position remains unrecorded. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `record_china_consensus_position` | known `negotiation_grace_notice` from `donald_trump_executive_interface`; known `bytedance_resolution_proposal` from `bytedance_platform_governance_interface` | Record only the Chinese delegation's qualified position after grace and corporate proposal are known. |
...
| --- | --- | --- |
| `record_china_consensus_position` | `negotiation` | `negotiation.china_delegation_position`: unrecorded → `qualified_consensus_position_recorded` |
...
- Normal: The gate publishes its own qualified position to ByteDance, the US delegation and Trump without writing another party's state.
- Missing information: The gate waits for both the grace notice and ByteDance proposal. If either is absent, its position remains unrecorded.
...
- Authority/adverse case: One side's position is not bilateral agreement or implementation and creates no ownership or operational effect.
- Perturbation: Withholding the corporate proposal leaves both delegation positions and the downstream resolution open.
...
The gate cannot bind ByteDance, the US delegation or either government, transfer algorithm rights, approve an operator, implement data hosting, or establish a final agreement alone. The final Draft relation names only Trump and ByteDance as consensus counterparties despite separate delegation rows. Current semantics require distinct P_9/P_10 positions before any ByteDance resolution record.
