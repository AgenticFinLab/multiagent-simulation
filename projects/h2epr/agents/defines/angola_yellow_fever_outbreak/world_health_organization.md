...
The admissible non-default intents are `record_initial_confirmation`, `document_cross_border_risk`, `convene_first_emergency_committee`, `coordinate_scaled_response`, `convene_second_emergency_committee`, `activate_regional_surveillance`. A declared coordinate and every state or message guard must match. Missing, stale, or adverse information yields `no_op` or a typed rejection, never a substitute act. Exact Rule rows and model settings remain outside this Definition.
...
The Definition is falsified if `world_health_organization` needs authority outside WHO's public monitoring, documentation, meeting-convening, response-coordination, and regional-surveillance choices represented by the Draft or if its modeled choice is causally inert under every meaningful perturbation.
