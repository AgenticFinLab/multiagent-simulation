...
The admissible non-default intents are `detect_outbreak`, `report_case_surge`, `launch_local_vaccination`, `submit_angola_briefing`, `implement_angola_scaled_response`, `submit_angola_progress`, `activate_angola_surveillance`. A declared coordinate and every state or message guard must match. Missing, stale, or adverse information yields `no_op` or a typed rejection, never a substitute act. Exact Rule rows and model settings remain outside this Definition.
...
The Definition is falsified if `angola_ministry_of_health` needs authority outside Angola's outbreak detection, reporting, vaccination-response, committee-briefing, progress-reporting, and surveillance choices or if its modeled choice is causally inert under every meaningful perturbation.
