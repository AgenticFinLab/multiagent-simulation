# Amperex Technology Limited Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0481.agent.atl_battery_supplier.v1` |
| Actor ID | `atl_battery_supplier` |
| Benchmark | H2EPR-0481, August 2016–January 2017 represented boundary |
| Representation | agent; supplier public-statement interface |
| Source ID | `P_6` |
| Primary choices | Issue the represented statement denying a direct link between its battery and the reported mainland incident. |
...
P_6 retains ATL's one exposed public statement. Its later appearance as investigation subject is passive and does not grant access to private manufacturing knowledge.

The statement neither establishes root cause nor proves battery safety; no supplier production, testing or remediation policy is simulated. The parent fixes no calibrated utility, personality,
...
| --- | --- | --- |
| draft_epg:S2/E5/P_6 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |
| draft_epg:S4/E8/P_6 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC002, SRC003, SRC005, SRC006, SRC009 and SRC011. Later material attributes a replacement-battery defect while earlier material denies a connection. The 2017 conclusion is not early observation and does not rewrite the September statement.
...
This agent may issue the represented statement denying a direct link between its battery and the reported mainland incident. It cannot act as another producer,
...
ATL cannot publish Samsung's conclusion, decide a recall or resolve the consumer dispute. Its denial may be accepted and later contested.
...
The selected statement waits for an actually delivered mainland incident report; source chronology alone is not message receipt. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `issue_battery_nonconnection_statement` | known `mainland_incident_report` from `china_regular_note7_purchasers` | Record ATL's represented denial statement, not a verified safety finding. |
...
| --- | --- | --- |
| `issue_battery_nonconnection_statement` | `domestic_statements` | `domestic_statements.atl_account`: unrecorded → `no_direct_connection_claim_recorded` |
...
- Normal: After the consumer report arrives, ATL can record and route its qualified denial statement to the affected consumer Population.
- Missing information: The selected statement waits for an actually delivered mainland incident report; source chronology alone is not message receipt.
...
- Authority/adverse case: ATL cannot publish Samsung's conclusion, decide a recall or resolve the consumer dispute. Its denial may be accepted and later contested.
- Perturbation: Missing report delivery removes ATL's statement and one prerequisite for the modeled consumer dispute without invalidating unrelated records.
...
The statement neither establishes root cause nor proves battery safety; no supplier production, testing or remediation policy is simulated. Later material attributes a replacement-battery defect while earlier material denies a connection. The 2017 conclusion is not early observation and does not rewrite the September statement.
