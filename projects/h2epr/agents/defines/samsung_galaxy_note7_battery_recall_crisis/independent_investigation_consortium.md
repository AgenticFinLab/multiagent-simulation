# UL–Exponent–TÜV Investigation Consortium Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0481.agent.independent_investigation_consortium.v1` |
| Actor ID | `independent_investigation_consortium` |
| Benchmark | H2EPR-0481, August 2016–January 2017 represented boundary |
| Representation | agent; composite third-party investigation and findings-publication interface |
| Source ID | `P_9` |
| Primary choices | Record the represented independent investigation and publish its qualified findings after receiving Samsung's commission. |
...
P_9 is a disclosed composite because the Draft presents three institutions as one organization group. It owns only the shared investigation records exposed by the source, not unobserved disagreements or laboratory microprocesses.

No raw test data, defect probability, supplier liability allocation or guarantee of independence is modeled. The parent fixes no calibrated utility, personality,
...
| --- | --- | --- |
| draft_epg:S4/E8/P_9 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |
| draft_epg:S4/E9/P_9 | Participant appearance and actor-local action rows | Draft content; not independently reconciled history |

Frozen anchors: SRC006 and SRC011, with the Draft supplying the named consortium. The frozen pages are retrospective and include disagreement with the reported conclusion. Findings are represented as published claims, not adjudicated physical truth.
...
This agent may record the represented independent investigation and publish its qualified findings after receiving Samsung's commission. It cannot act as another producer,
...
Samsung cannot impersonate this finding owner. Conducting the investigation does not itself publish findings, and publication does not force Samsung's later report.
...
Without an actually delivered commission, both consortium rows wait; without delivered findings, Samsung's final report row remains unavailable. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `conduct_independent_investigation` | known `investigation_commission` from `samsung_electronics` | Record the represented third-party investigation process after commission delivery. |
| `publish_independent_findings` | `investigation.third_party_process` = `recorded` | Record the consortium's published findings as claims, not adjudicated physical truth. |
...
| --- | --- | --- |
| `conduct_independent_investigation` | `investigation` | `investigation.third_party_process`: unrecorded → `recorded` |
| `publish_independent_findings` | `investigation` | `investigation.third_party_findings`: unrecorded → `published_claim_recorded` |
...
- Normal: A delivered commission enables the investigation record; its accepted state permits a later findings publication and message to Samsung.
- Missing information: Without an actually delivered commission, both consortium rows wait; without delivered findings, Samsung's final report row remains unavailable.
...
- Authority/adverse case: Samsung cannot impersonate this finding owner. Conducting the investigation does not itself publish findings, and publication does not force Samsung's later report.
- Perturbation: Delaying publication beyond Samsung's bounded report window leaves a valid open final-report endpoint with complete transport accounting.
...
No raw test data, defect probability, supplier liability allocation or guarantee of independence is modeled. The frozen pages are retrospective and include disagreement with the reported conclusion. Findings are represented as published claims, not adjudicated physical truth.
