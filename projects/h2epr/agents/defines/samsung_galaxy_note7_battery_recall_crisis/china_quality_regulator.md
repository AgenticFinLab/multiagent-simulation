# China Quality-Supervision Regulator Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0481.agent.china_quality_regulator.v1` |
| Actor ID | `china_quality_regulator` |
| Benchmark | H2EPR-0481, August 2016–January 2017 represented boundary |
| Representation | agent; limited-recall requirement issuance interface |
| Source ID | `P_4` |
| Primary choices | Issue the represented requirement that Samsung file a recall plan for 1,858 test units. |
...
P_4 represents the Draft's named national product-quality regulator for one bounded requirement. It does not absorb civil-aviation, court, media or consumer authority.

No general enforcement regime, fine, finding of defect, nationwide full-recall order or completed compliance is modeled. The parent fixes no calibrated utility, personality,
...
Frozen anchors: SRC004, SRC007 and SRC011. The dataset alternates between interview, consultation and requirement language; the executable record is a qualified recall-plan requirement, not a broader adjudication.
...
This agent may issue the represented requirement that Samsung file a recall plan for 1,858 test units. It cannot act as another producer,
...
Issuance does not itself create Samsung's filing or compensate consumers. Another actor invoking this intent must be rejected.
...
The Rule may issue within its bounded regulatory window; if its message is not delivered, Samsung cannot use receipt as a guard. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `require_test_unit_recall_plan` | source-bounded availability and own record not yet made | Record the qualified requirement for a 1,858-unit recall filing. |
...
| --- | --- | --- |
| `require_test_unit_recall_plan` | `regulation` | `regulation.test_unit_requirement`: unrecorded → `issued` |
...
- Normal: The regulator records the requirement and transport delivers a qualified notice to Samsung before the manufacturer may record the limited recall.
- Missing information: The Rule may issue within its bounded regulatory window; if its message is not delivered, Samsung cannot use receipt as a guard.
...
- Authority/adverse case: Issuance does not itself create Samsung's filing or compensate consumers. Another actor invoking this intent must be rejected.
- Perturbation: A delayed regulatory notice shifts or prevents the manufacturer row without changing the regulator's accepted issuance record.
...
No general enforcement regime, fine, finding of defect, nationwide full-recall order or completed compliance is modeled. The dataset alternates between interview, consultation and requirement language; the executable record is a qualified recall-plan requirement, not a broader adjudication.
