# Symantec Attribution Publisher Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic parent | `h2epr.0616.agent.symantec_attribution_publisher.v1` |
| Actor ID | `symantec_attribution_publisher` |
| Benchmark | H2EPR-0616, June 2018–March 2019 represented response boundary; affected-record cohort dates begin May 2015 |
| Representation | agent; public threat-attribution report interface |
| Source ID | `P_9` |
| Primary choices | Publish the represented March 2019 report attributing the breach to Whitefly as a qualified claim. |
...
P_9 owns only the public report record exposed in E8. It remains organizational and does not simulate researchers, telemetry, proprietary evidence or internal attribution confidence.

The publisher cannot prove state sponsorship, rewrite 2018 participant knowledge, issue a legal attribution, impose penalties, or determine the technical attack path. The parent fixes no calibrated utility, personality,
...
Frozen anchors: SRC011. The frozen source is a retrospective secondary report. Its attribution is modeled as a published claim rather than ground-truth identity.
...
This agent may publish the represented March 2019 report attributing the breach to Whitefly as a qualified claim. It cannot act as another producer,
...
Publishing the claim does not alter earlier decisions or make the representation gate's private structural identity an earlier public fact.
...
The row is independent of the COI/penalty chain. If omitted, public attribution remains unrecorded and no environment default fills it. Memory persists across this bounded event without a
...
| --- | --- | --- |
| `publish_whitefly_attribution_report` | source-bounded availability and own record not yet made | Record the March 2019 Whitefly attribution as a published claim, not verified identity truth. |
...
| --- | --- | --- |
| `publish_whitefly_attribution_report` | `attribution` | `attribution.public_report`: unrecorded → `whitefly_claim_recorded` |
...
- Normal: The publisher records the qualified attribution near the horizon and sends the public report to the two affected organizations.
- Missing information: The row is independent of the COI/penalty chain. If omitted, public attribution remains unrecorded and no environment default fills it.
...
The publisher cannot prove state sponsorship, rewrite 2018 participant knowledge, issue a legal attribution, impose penalties, or determine the technical attack path. The frozen source is a retrospective secondary report. Its attribution is modeled as a published claim rather than ground-truth identity.
