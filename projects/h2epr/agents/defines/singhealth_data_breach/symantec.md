# Symantec — Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic ID | `h2epr.0616.agent.symantec.v0.4` |
| Runtime actor | `symantec` |
| Benchmark event | SingHealth Data Breach (`H2EPR-0616`) |
| Source participant | `P_9` — Symantec (赛门铁克) |
| Representation | `agent` |
| Event role | public attribution issuer |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. The underlying threat-intelligence inference is outside the dataset package.
...
Role: public attribution issuer.

Authority boundary: May publish the exposed Whitefly attribution.
