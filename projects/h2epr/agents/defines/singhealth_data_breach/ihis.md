# Integrated Health Information Systems — Agent Definition

## 1. Model overview

| Field | Account |
|---|---|
| Semantic ID | `h2epr.0616.agent.ihis.v0.4` |
| Runtime actor | `ihis` |
| Benchmark event | SingHealth Data Breach (`H2EPR-0616`) |
| Source participant | `P_3` — Integrated Health Information Systems Pte Ltd (IHiS) |
| Representation | `agent` |
| Event role | data operator and response owner |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. Technical telemetry and staff-level actions are compressed into exposed transitions.
...
Role: data operator and response owner.

Authority boundary: May verify, contain, and implement operator improvements.
...
- `entities.reforms.operator_status`

## 6. Admissible decision semantics

The backend may choose only:

- `implement_operator_improvements`
- `no_op`
- `verify_and_contain`
