...
| Field | Account |
|---|---|
| Semantic ID | `h2epr.0616.agent.committee_of_inquiry.v0.4` |
| Runtime actor | `committee_of_inquiry` |
| Benchmark event | SingHealth Data Breach (`H2EPR-0616`) |
| Source participant | `P_7` — SingHealth Data Breach Committee of Inquiry |
| Representation | `agent` |
| Event role | investigative body |
| Dataset exposure | Full Draft exposed; construction practice only |

## 2. Benchmark participant and representation

This semantic parent retains the participant as an autonomous action-bearing boundary because one or more Draft episodes assign it an exposed transition. Evidence weighing inside the inquiry is not modeled.
...
Authority boundary: May produce the root-cause findings exposed by the Draft EPG.
