# H2EPR-0481: Samsung Galaxy Note7 Battery Recall Crisis

This coordination entry indexes the accepted H2EPR-0481 research,
configuration, bounded lineage, full-roster Rule execution, replay, and
generated-graph releases. The Frame fixes the question, causal boundary, and
temporal contract; later releases preserve those authorities while closing
the deterministic engineering path.

## Identity and construction contract

| Field | Value |
|---|---|
| Event identifier | `H2EPR-0481` |
| Title | Samsung Galaxy Note 7 Battery Recall Crisis |
| Domain / category | Science & Engineering / Engineering & Product Safety |
| Stable slug | `samsung_note7_battery_recall` |
| Frame version | `v0.1` |
| Status | `PASS_FULL_ROSTER_RULE_RUN_AND_GRAPH_v0.1` |
| Construction mode | `DRAFT_SEEDED_REFERENCE_BLIND` |
| Current authorization | third-event engineering cycle complete; no calibration, evaluation, policy, or new-event scope is implied |
| Phase endpoint | [accepted run and generated graph `v0.1`](../../execution/samsung_note7_battery_recall/run-and-graph-v0.1/) and [three-event conformance successor `v0.2`](../../execution/cross-event-conformance-v0.2/) |
| Review | event phases self-reviewed in the independent execution window; final review remains with the designated supervisor window |
| Method baseline | H2EPR mainline at commit `592fb706bdf6c8720a0884f1d5ee76d237fe4ebd`, tree `03d3062742d3643688c60cd36f74b0476cc0275e`; the maintained workflow, architecture, publication standard, Event Build Brief, phase closeout checklist, and historical-evidence-research Skill govern this Frame |

The permitted seeded inputs are fixed by content identity:

| Input | SHA-256 | Use in this phase |
|---|---|---|
| `event_spec.json` | `d61821a6b9042b7645b215300bce2ebab5e3933eae17b00421352a81121aed33` | Event identity only |
| `frozen_evidence.json` | `c57db941c601de94e0f8826bd74537b2d4b4bc08c94d1ea3d7cec249b1a3c325` | Claim-family discovery; every material proposition was independently adjudicated |
| `draft_epg.json` | `9883cae0a41d1345de43efa4658b8d4162fdf5843dadf94da36be5a6e9a26254` | Disclosed process seed; no stage, episode, participant, mechanism, or outcome assertion was accepted on its authority |

The complete evidence authority for the accepted Frame is
[frame-evidence-v0.1.md](./frame-evidence-v0.1.md). The two files form one
accepted Frame review unit. The later
[participant-evidence record](participant-evidence-v0.1.md) owns role-specific
claims and participant-time limits.

## Event question and claim boundary

> Within the bounded Galaxy Note7 product-safety crisis, how did incident and
> defect signals move through corporate investigation and escalation,
> supplier and replacement-device handling, carrier and retailer action, and
> consumer-safety and transport regulation to transform a launched product
> into a global stop-sale, production discontinuation, and expanded recall?
> How did the timing and authority of the first recall, replacement response,
> renewed incidents, and cross-organizational restrictions alter that event
> process?

The purpose of the event is to support engineering-and-method mechanism
coverage: it asks how differently situated decision interfaces received
signals, exercised authority, and changed the reachable product-safety
process. The current claim is not calibration, historical fit, held-out
performance, defect-cause proof, liability allocation, recall effectiveness,
scientific validity, predictive accuracy, or universal generality. A source's
report of an incident or investigation finding remains a source-attributed
report; it is not silently promoted into an adjudicated physical cause.

## Temporal contract

| Layer | Interval | Frame use |
|---|---|---|
| Outer evidence interval | `2016-08-19_TO_2017-01-23` | Bounds the launch-to-retrospective evidence record |
| Core modeled crisis interval | `2016-08-19_TO_2016-10-15` | Bounds any future event execution |
| Acute escalation interval | `2016-09-02_TO_2016-10-15` | Concentrates the sales stop, replacement, renewed-incident, discontinuation, recall-expansion, and transport-restriction sequence |
| Post-crisis observation | `2016-10-16_TO_2017-01-23` | Retrospective observation and bounded remediation context only |

Four clocks must remain distinct: the time an event occurred or an order
became effective; the time information could have been available to a
particular participant; the source's publication time; and the research
access time. Public posting does not prove that a participant received or
read a source. Conversely, a later source that recounts an earlier act does
not make the later consolidation available at the earlier date.

The Samsung investigation announcement dated 22 January on the U.S. page and
23 January in its Seoul dateline is post-crisis evidence. Its findings and the
remediation described with them are categorically unavailable to actors in
the 2016 modeled interval. No future participant artifact may expose that
material, the expanded recall, the production halt, or the final transport
ban before the corresponding historical delivery gate.

## Exposure and protected-input declaration

This Frame was built in `DRAFT_SEEDED_REFERENCE_BLIND` mode. Before any
target fixture was opened, the Builder declared no prior exposure in this
window to H2EPR-0481 Reference, held-out, or evaluation content. The only
target files read were the three permitted inputs identified above. The
Reference EPG, held-out suffix, and evaluation-only content were not opened,
parsed, searched, indexed, hashed through a content reader, or used. A
Reference filename or inventory identity is not treated as content exposure.

The completed Frame authorization permitted public-source research and the
two initial event-specific Markdown records only. The Frame Builder stopped at
that boundary. Independent supervisor review returned one bounded
causal-ownership correction and accepted the Frame only after the two
transport-issuance gates were separated from their institutional lifecycles.

A later authorized participant phase adopted three additional official pages,
resolved every representation gate, authored four Agent Definitions and four
Population Models, and produced the hash-pinned Roster Definition release. It
did not open protected content or add Scenario, mapping, configuration,
implementation, simulation, generated-graph, calibration, or evaluation
semantics to the participant products.

## Bounded causal scope

The event begins with a commercially launched device population and incoming
incident signals. Those are conditions upon which organizations act, not a
prewritten route to the known outcome.

| Causal class | Included content | Boundary condition |
|---|---|---|
| Initial or exogenous context | Commercial launch in selected markets; devices and battery cells already in circulation; incident reports and physical hazard realizations as they arrive | A report is a signal, not proof of mechanism; device failures must not be generated merely to force the historical outcome |
| Endogenous choices | Samsung investigation, sales/shipment and replacement decisions, supplier-facing inspection, public safety communications, production decision; CPSC warning and recall decisions; the authority-specific choice to issue the CAAC warning and the U.S. DOT/FAA/PHMSA emergency order, each behind its own representation gate; carrier, retailer, consumer, airline, and airport responses when represented | Choice ownership must stay with the historically authorized interface; no later action is written into an earlier choice, and a transport lifecycle may not absorb its unresolved issuance decision |
| Scenario or institutional processes | Signal delivery; jurisdiction-specific recall procedure; replacement inventory and remedy availability; retail and carrier processing; after a transport action is validly issued, its publication, effective-time transition, routing and delivery, carrier or airport obligations, enforcement, and observable result production; physical-device state transitions | These processes may constrain, deliver, or adjudicate choices but may not make an unresolved warning/order issuance choice, impersonate an Agent, or infer undocumented compliance |
| Results | A public corporate sales stop and replacement program; formal U.S. recall; replacement-device reports and renewed investigation; requested global partner stop; production halt; expanded U.S. recall; transport restrictions effective by 15 October | Announcement, legal status, attempted remedy, completed return, and population-level effectiveness remain distinct results |
| Excluded aftermath | Litigation and liability; market-share, financial, brand, environmental, and disposal effects; return-rate claims; post-15 October campaigns; post-crisis design reform beyond bounded retrospective context | These matters cannot be smuggled into the core event as rewards, causes, or known outcomes |

The evidence supports a bounded transition hypothesis: incident signals
prompted corporate investigation and a replacement program; formal consumer
safety authority then changed the U.S. recall state; replacement-device
signals reopened the safety question; corporate, intermediary, recall, and
transport authorities subsequently produced overlapping restrictions through
authority-specific issuance decisions and downstream institutional
lifecycles. This is a sourced process synthesis, not proof that any one action
caused the next or that all jurisdictions and intermediaries behaved
uniformly.

## Accepted causal-role map

The [research roster](../../agents/rosters/samsung_note7_battery_recall.md)
owns the full dispositions and reopening conditions. Historical prominence
does not itself make an entity an Agent, and the release sets no roster-size
target.

| Historical entity or process | Accepted disposition | Causal boundary |
|---|---|---|
| Samsung Electronics product-safety decision interface | **deep Agent Definition** | A narrow outward interface owns investigation, product-flow, replacement, partner, communication, and production intents without becoming a whole-company mind. |
| Samsung regional sales, service, and communications units | **standard Population Model** | Only evidence-supported jurisdictional units own local partner, remedy, and communication intents; the Singapore record is not generalized to every region. |
| Battery suppliers and supplier-facing technical teams | **deferred Scenario/investigation context** | Supplier-facing inspection may be routed, but 2016 autonomy, knowledge, fault, and causally necessary choice remain unsupported. |
| U.S. Consumer Product Safety Commission recall interface | **deep Agent Definition** | A bounded authority interface owns warning, information/remedy review, formal-recall, and expansion intents; legal state and effectiveness remain external. |
| Wireless carriers and retail outlets | **standard Population Model** | Unit-local sales, notice, inventory-request, exchange, and refund intents remain distinct from inventory and completion. |
| Galaxy Note7 owners and prospective consumers | **standard Population Model** | Individual purchase, use, power-down, reporting, exchange, and refund choices remain unit-local. |
| CAAC transport-warning issuance decision interface | **standard Agent Definition** | A narrow CAAC interface owns information, warning, and qualification intents; unresolved internal governance remains a split trigger. |
| CAAC warning lifecycle after issuance | **Scenario or institutional process** | Publication, routing, delivery, operator duties, enforcement conditions, and results begin only after valid issuance. |
| U.S. DOT/FAA/PHMSA emergency-order issuance decision interface | **standard Agent Definition** | A Secretary-level interface owns information, qualification, and issuance intents while bounded FAA/PHMSA inputs remain external observations. |
| U.S. emergency-order lifecycle after issuance | **Scenario or institutional process** | Publication, the 15 October effective-time transition, routing, duties, enforcement, petition, and results begin only after valid issuance. |
| Passenger airlines, airports, and cargo operators | **standard Population Model** | Jurisdiction- and function-local communication, handling, denial, escalation, and permitted stricter-measure intents remain heterogeneous. |
| Device and battery population, incident arrival, and physical hazard realization | **initial or exogenous context** | They provide evolving conditions and signals. They are not Agents, and their timing must be independently sourced or modeled without outcome forcing. |
| Media coverage, courts, investors, competitors, and later remediation programs | **excluded** | They are outside the question's minimum causal boundary or postdate the core interval. |

## Shared semantics and ownership skeleton

The following concepts will be interpreted by more than one future product
or have authority that could otherwise become ambiguous. This skeleton fixes
their meanings without entering Scenario or participant design.

| Shared concept | Semantic owner or authority | Minimum invariant |
|---|---|---|
| Incident report / safety signal | Evidence record owns provenance; a future Scenario owns delivery state | `reported`, `received`, `investigated`, and `verified` are different states; arrival time and recipient are explicit |
| Investigation state / finding | The investigating institution owns its internal state; evidence owns public statements | A statement about a finding is attributed and time-gated; the 2017 result is never a 2016 observation |
| Product-flow status | The organization with operational authority owns each state | Sales, shipments, exchanges, production, and completed returns are separate variables rather than one `recall` flag |
| Recall status | Jurisdiction-specific consumer-safety authority plus the initiating firm | Samsung's 2 September program, CPSC's 15 September formal recall, and CPSC's 13 October expansion are distinct states |
| Device class | Product-identity and remedy records | Original device, replacement device, and green-icon replacement are explicit labels; a label is not a safety guarantee |
| Remedy | Firm/intermediary offers; consumer choice; process adjudication | Offer, eligibility, availability, selection, handoff, refund, exchange, and completion remain separable; effectiveness is out of claim scope |
| Restriction-issuance choice | The named authority's decision interface, or a bounded representation gate while that interface remains unresolved | Issuance, withholding, and escalation are autonomous-choice questions distinct from an issued action's lifecycle; CAAC and U.S. transport issuance remain separate gates |
| Issued-restriction lifecycle | Scenario or institutional process after a valid issuance result | Publication, effective time, routing and delivery, carrier or airport obligations, enforcement, and observable results retain issuer and jurisdiction but do not recreate the issuance choice |
| Supplier / battery lineage | Evidence and product-state provenance | Lineage cannot imply cause, knowledge, negligence, or liability; later diagnostic categories are future-only |
| Availability clock | Evidence-delivery boundary | Event time, participant-time availability, source time, and research-access time are recorded separately |

## Accepted semantic work package and retained limits

The accepted semantic package now consists of the event entry, Frame and
participant evidence, research roster, semantic skeleton, four Agent
Definitions, four Population Models, one shared interface, and the
[Roster Definition release](../../releases/samsung_note7_battery_recall/roster-definition-v0.1/).
The manifest pins each input by content identity.

The participant phase resolves both transport issuance gates without changing
their downstream ownership. It retains explicit reopening conditions for
internal Samsung, CPSC, CAAC, and DOT/FAA/PHMSA boundaries; limits regional
instantiation to supported units; and defers suppliers absent contemporaneous
autonomy evidence. These are structural limits, not hidden parameters.

Later work must stop if it would require protected content, a new causal
scope, unsupported organizational authority, a silent roster change, or a
shared H2EPR contract change. The accepted
[consolidated mapping](../../agents/bindings/samsung_note7_battery_recall/consolidated/),
[Event Scenario Definition](../../scenarios/samsung_note7_battery_recall/definition-v0.1/),
and [Scenario Configuration](../../configs/samsung_note7_battery_recall/scenario-configuration-v0.1/)
derive from the exact Roster release. Static admission preserves their
non-executable boundary.

## Reproducibility and execution records

- [configuration admission](../../configs/samsung_note7_battery_recall/configuration-admission-v0.1/)
- [bounded lineage binding](../../agents/bindings/samsung_note7_battery_recall/samsung-regional-outlet-consumer-v0.1/)
- [bounded lineage conformance](../../scenarios/samsung_note7_battery_recall/lineage-conformance-v0.1/)
- [Policy Realization](../../execution/samsung_note7_battery_recall/policy-realization-v0.1/)
- [full-roster executable package](../../execution/samsung_note7_battery_recall/full-roster-rule-v0.1/)
- [run and generated graph](../../execution/samsung_note7_battery_recall/run-and-graph-v0.1/)
- [three-event execution conformance](../../execution/cross-event-conformance-v0.2/)

## Phase closeout

The accepted Frame remains the event-boundary authority. Participant evidence
adds only the role resolution needed to produce the eight qualitative models.
All products retain the four-clock firewall, known-outcome exposure, explicit
alternatives and falsifiers, finite intent lifecycles, and the separation of
participant intent from institutional or physical result. The release adds no
new shared schema or H2EPR contract.

The authoring-exposed substantive review closed the original-source,
representation, lifecycle, deep-case, and cross-region findings before
release. Structural checks, release pin verification, strict JSON, checksum,
trace, authoritative replay, deterministic-pair, graph-reference, and
publication checks pass. The optional MASim history-dependency limitation is
recorded in the run review. Final independent supervisor review is deferred
until the completed third-event cycle is handed back.

**Current disposition:** Frame, participant definition, roster, mapping,
Scenario Definition, qualitative configuration, static admission, bounded
lineage, Policy Realization, full-roster executable successor, two-run
determinism, authoritative replay, generated EPG, and three-event conformance
are complete with recorded limitations. **Next legal action:** independent
final supervisor review; no scientific or new research cycle starts
automatically.
