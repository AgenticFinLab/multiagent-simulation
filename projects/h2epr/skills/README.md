...
`references/guide.md` method guide. High-judgment Skills may route from that
guide to focused method references, complete examples, and annotated failures.
Read only the package selected for the current responsibility, then follow the
routes named by its entry point. This progressive disclosure keeps routine
orchestration compact while preserving appendix-grade method detail.
...
do not belong in a reusable Skill. A worked example must identify itself as
illustrative, use synthetic or already admitted material, and remain
non-normative.
