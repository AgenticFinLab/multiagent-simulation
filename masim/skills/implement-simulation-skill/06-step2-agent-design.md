...
5. **Handbook structural conformance + §6 three-PASS.** This hook has two parts, run in order:
   ...
   **5a. Tool-backed structural gate.** Run `scripts/audit_agent_handbook.py` against every pool profile touched by this scenario (typically the profiles named in `simulation-bases.md §4`, or the full domain via `--domain {domain}`). The script checks `agent-design-skill.md §2` canonical section order, §3 section-by-section structural requirements (Summary row labels, Theory sub-block labelled lines, Parameters 8-column table, Worked Examples case count, IF-THEN sanity-bound count, Provenance rows, etc.), and §3.6.0 I/O Contract block presence. Any FAIL halts the polish run; remediate the profile structurally before proceeding. Backing tool: `scripts/audit_agent_handbook.py --profile examples/AGENT_POOL/{domain}/{agent}.md` (single profile) or `--domain {domain}` (whole domain). The script's exit code is the gate: 0 = proceed, 1 = halt.
