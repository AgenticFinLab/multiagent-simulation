...
| Source                                        | Used for                                                                                                                                                                                                                                                                                                                                                        |
|-----------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Target §7 Agent Roster (rows)                 | CREATE flow: one-to-one seed for each `simulation-bases.md §4.{N}` block (target §7 → bases §4). POLISH flow: MUST agree with existing `simulation-bases.md §4.{N}` header names; §4 is authoritative (see `polish-simulation-pipeline.md §0.1`). §7 drift is patched via `define-simulation-scenario-skill.md §9.3 revise mode`, never by editing §4 directly. |
| `simulation-bases.md §4.{N}` block headers    | POLISH flow: authoritative archetype-name set. Every implementation identity in every variant's `players.yml` MUST canonicalize (via `_canonical_archetype`) to one of these header stems (kebab-normalized). This is the input to Hook 6a below.                                                                                                               |
...
5. **Handbook structural conformance + §6 three-PASS.** This hook has two parts, run in order:

   **5a. Structural gate (manual).** For every pool profile touched by this scenario (typically the profiles named in `simulation-bases.md §4`, or the full domain), walk the profile line-by-line and verify: `agent-design-skill.md §2` canonical section order, §3 section-by-section structural requirements (Summary row labels, Theory sub-block labelled lines, Parameters 8-column table, Worked Examples case count, IF-THEN sanity-bound count, Provenance rows, etc.), §3.6.0 I/O Contract block presence, **and a whole-file placeholder scan** — no `TODO`, `TBD`, `FIXME`, `XXX`, `PLACEHOLDER`, `Status: stub`, `auto-generated placeholder`, `fill this section`, or `insert here` marker may exist outside a fenced code block. Any FAIL on any check family halts the polish run; remediate the profile structurally and author any placeholder content before proceeding. **No form of TODO, placeholder, or stub content is permitted in a conformant profile — the polish pipeline MUST NOT skip or defer authoring any section.**

   **5b. Semantic three-PASS.** After 5a is green, for every agent run `agent-design-skill.md §6` Validation Checklist three consecutive times; any FAIL resets the count. 5a handles the structural items the tool can check; 5b handles the remaining semantic-quality items (is the Mathematical Formulation implementable? is the Empirical Evidence an effect size? does each mechanism step trace to §3.4?) that require human/LLM judgment.
6. **Cross-variant archetype-set parity, anchored to `simulation-bases.md §4`.** This hook has two parts, run in order:

   **6a. Canonical archetype set = `§4.N` block headers of `simulation-bases.md`** (kebab-normalized). Parse `examples/{Scenario}/simulation-bases.md` for all `### §4.N` headers (where `N` is a positive integer); strip the `§4.N ` prefix; convert each header name to `kebab-case`. This produces the **canonical archetype set** for the scenario. Every top-level identity in every variant's `configs/{Scenario}/{V}/players.yml` (excluding reserved keys such as `environment` / `market` / `knowledge`) MUST satisfy `_canonical_archetype(identity).replace("_", "-")` ∈ canonical set. Violations halt via `AskUserQuestion` with three canonical options: (a) **rename identities/classes to match §4 headers** — the default, no theory change, no target-file edit, standard renames across `players.yml` / `topology.yml` / `players.py` / `__init__.py` / `explain.md`; (b) **§4 wins ⇔ push the archetype rename into §4 via `define-simulation-scenario-skill.md §9.3 revise mode`** — requires updated citations, formulas, and empirical evidence in the §4.N block body, and a new §0 CHANGELOG entry; (c) **intentional per-variant divergence** — recorded as a §4 sub-appendix and a new target §7 row via revise mode. Option (a) is the standard fix during a polish run; options (b) and (c) require the polish run to halt and hand off to the define skill.

   **6b. Cross-variant equality** (after 6a passes). Compute the archetype set per variant as `{ _canonical_archetype(identity) : identity ∈ configs/{Scenario}/{V}/players.yml top-level keys, excluding reserved keys }`. All built variants of the same scenario (per target §10.1) MUST have equal archetype sets — the pool profiles and icons are archetype-level assets shared across variants (the variant prefix is stripped by `_canonical_archetype()` before asset lookup). On mismatch after 6a is green, halt via `AskUserQuestion` with the same three-option resolution surface as 6a; option (a) here means "align the outlier variant to the majority, which itself must already conform to §4". Never silently paper over parity gaps by generating extra assets.

   **How to execute manually.** For 6a, parse `examples/{Scenario}/simulation-bases.md` for all `### §4.N` headers, kebab-normalize each name, and check every top-level identity in `configs/{Scenario}/{V}/players.yml` (excluding reserved keys) against the canonical set. For 6b, compute the archetype set per variant the same way and diff across variants. During create flow only 6b applies (variant-set parity, using `players.yml` as the anchor); during polish flow both 6a and 6b apply (anchored to `simulation-bases.md §4`).
