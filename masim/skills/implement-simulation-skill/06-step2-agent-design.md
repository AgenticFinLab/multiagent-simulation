...
5. **Handbook structural conformance + §6 three-PASS.** This hook has two parts, run in order:
   ...
   **5a. Tool-backed structural gate.** Run `scripts/audit_agent_handbook.py` against every pool profile touched by this scenario (typically the profiles named in `simulation-bases.md §4`, or the full domain via `--domain {domain}`). The script checks `agent-design-skill.md §2` canonical section order, §3 section-by-section structural requirements (Summary row labels, Theory sub-block labelled lines, Parameters 8-column table, Worked Examples case count, IF-THEN sanity-bound count, Provenance rows, etc.), §3.6.0 I/O Contract block presence, **and a whole-file placeholder scan** (`--check todo`) that flags any `TODO`, `TBD`, `FIXME`, `XXX`, `PLACEHOLDER`, `Status: stub`, `auto-generated placeholder`, `fill this section`, or `insert here` marker outside a fenced code block. Any FAIL on any check family halts the polish run; remediate the profile structurally and author any placeholder content before proceeding. Backing tool: `scripts/audit_agent_handbook.py --profile examples/AGENT_POOL/{domain}/{agent}.md` (single profile) or `--domain {domain}` (whole domain). The script's exit code is the gate: 0 = proceed, 1 = halt. **No form of TODO, placeholder, or stub content is permitted in a conformant profile — the polish pipeline MUST NOT skip or defer authoring any section.**
...
6. **Cross-variant archetype-set parity, anchored to `simulation-bases.md §4`.** This hook has two parts, run in order:
   ...
   **Backing tool.** `scripts/audit_agent_naming.py --scenario {ScenarioName} --anchor simulation-bases` implements both 6a and 6b; `--anchor players` (the default) skips 6a and runs 6b only, which is the create-flow contract.
