...
This folder is the **authoritative creation guide** for building financial
multi-agent simulations in this project. Each phase of the methodology is
expanded into a dedicated, deeply specified file.

Reading through these files in order gives a complete, step-by-step
methodology for:

1. Defining a financial phenomenon
2. Grounding it in academic and empirical research
3. Designing a rigorous investor taxonomy whose per-investor specifications
   conform to the **Universal Agent Design Handbook** at
   `masim/format/agent-design-skill.md`, instantiated for the financial
   domain through `masim/format/agent-design-finance.md`
...
validation) is governed by `agent-design-skill.md` (domain-neutral core)
together with `agent-design-finance.md` (financial-domain instantiation).
Whenever this guide and those files overlap, the handbook + companion
prevail.
...
| `02-root-documents-spec.md`    | Spec      | `simulation-bases.md` (9-section) + `analysis-bases.md` (7-section) full specifications. §4 defers to the **Universal Agent Design Handbook** (`masim/format/agent-design-skill.md`) and the **Financial Domain Companion** (`masim/format/agent-design-finance.md`) for every per-investor specification. |
...
Taxonomy** specifies how each investor entry conforms to the Universal
Agent Design Handbook (`masim/format/agent-design-skill.md`) under the
embedded-form header levels, with financial-domain row labels and value
palettes drawn from the Financial Domain Companion
(`masim/format/agent-design-finance.md`).
...
```
masim/format/agent-design-skill.md   ← Universal handbook: per-agent
        │                              intrinsic specification standard
        │ governs every investor entry under §4 of
        │ simulation-bases.md (any scenario domain).
        │
        ├─→ masim/format/agent-design-finance.md
        │        ← Financial domain companion: row labels, value
        │          palettes, and worked instantiation examples for
        │          market-trading scenarios.
        │
        ↓
masim/format/create-example-skill/   ← This folder: creation methodology
        │                              for financial multi-agent simulations
        │ drives
        ↓
...
section to `masim/format/agent-design-skill.md` (the canonical 12-section
format), with header levels shifted to embedded form per
`02-root-documents-spec.md §4.0` and Action Space labels instantiated for the
market-trading domain per `02-root-documents-spec.md §4.1`. The handbook
requires:

1. Title — sentence-cased role phrase, not a class identifier
2. Summary — 7 fixed rows (Archetype, Theory Family, Market Role, Time
   Horizon, Risk Tolerance, Information Asymmetry, Determinism)
...
10. Validation and Calibration — with ≥1 Ablation Hook
11. Academic References
12. Design Provenance and Versioning
