"""
Algo market entrypoint (Terminal #2).
...
def main():
    ...
    started = launch_market_proxies(
        ...
        proxy_config={
            "api_keys": simulation_config["extras"].get("api_keys", {}),
            "env_overrides": simulation_config["extras"].get("env_overrides", {}),
        },
    )

    logging.info("%s Launched #%d markets",
                 log_tag.START_TAG,
                 len(started)
                 )
    logging.info(
