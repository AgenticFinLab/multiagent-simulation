...
class MarketReactiveInvestor(GeneralInvestor):
    ...
    def _extract_signals(self, decision_content: Dict) -> Dict[str, float]:
        ...
        price = decision_content["clearing"]["price"]
        # Trading volume (absolute value to ensure positive)
        volume = abs(decision_content["clearing"]["volume"])
        # Bid-ask spread (market liquidity cost)
        spread = decision_content["clearing"]["spread"]
        # Realized volatility (risk measure from market)
        # Default to 0 if not provided by market
        volatility = decision_content["additions"].get("realized_volatility", 0.0)
        # Excess demand (quantity demanded - quantity supplied)
        # Positive = buy pressure, Negative = sell pressure
        excess_demand = decision_content["clearing"]["excess_demand"]
